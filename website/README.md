...
Static site (Vite + React) for the TQ-Claw product. Built output is served with a minimal Node server that supports SPA fallback (e.g. direct access to `/docs/channels`).
...
- **App name**: `tqclaw-website` in `ecosystem.config.cjs` (used in `pm2 logs/restart`).

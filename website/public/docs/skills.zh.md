...
应用从工作目录下的 `skills` 目录（默认 `~/.tqclaw/active_skills/`）加载能力：每个子目录中只要包含一份 `SKILL.md`，即会被识别为一个 Skill 并加载，无需额外注册。
...
| Skill 名称                   | 说明                                                                                                                                        | 来源                                                           |
| ---------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------- |
| **cron**                     | 定时任务管理。通过 `tqclaw cron` 或控制台 Cron Jobs 创建、查询、暂停、恢复、删除定时任务，按时间表执行并把结果发到频道。                     | 自建                                                           |
...
首次运行时会从包里把 **Cron** 同步到 `~/.tqclaw/active_skills/cron/`。它提供「按时间表执行任务并把结果发到频道」的能力；具体任务的增删改查用 [CLI](./cli) 的 `tqclaw cron` 或控制台 **Control → Cron Jobs** 完成，不需要手写 cron 以外的配置。

常用操作：

- 创建任务：`tqclaw cron create --type agent --name "xxx" --cron "0 9 * * *" ...`
- 查看列表：`tqclaw cron list`
- 查看状态：`tqclaw cron state <job_id>`
...
1. 在 `~/.tqclaw/customized_skills/` 下新建一个目录，例如 `my_skill`。
...
应用启动时会将内置 Skill 与 `~/.tqclaw/customized_skills/` 中的自定义 Skill 合并同步到 `~/.tqclaw/active_skills/`，同名时自定义优先。你在 `customized_skills` 中新加的目录不会被覆盖；内置 Skill 只会在 `active_skills` 中缺失时复制一次，已存在则不会覆盖。

...
**控制台** 是 TQ-Claw 内置的 Web 管理界面。运行 `tqclaw app` 后，在浏览器中打开
...
在这里创建和管理 TQ-Claw 按时间自动执行的定时任务。

![定时任务](https://img.alicdn.com/imgextra/i1/O1CN01B83KJC1LptPwniEDa_!!6000000001349-2-tps-3822-2070.png)

**创建新任务：**

> 如果定时任务没有创建成功，可以参考 [FAQ](https://tqclaw.agentscope.io/docs/faq) 的 **定时任务错误排查** 寻找原因

创建定时任务的 **最简单的方式是直接与 TQ-Claw 对话**，让他为你创建。例如你想在钉钉上收到喝水提醒，则在钉钉上与 TQ-Claw 对话：“帮我创建一个定时任务，每隔 5 分钟提醒我喝水。”创建完成后，可以在控制台的定时任务页面看到创建好的定时任务。
...
2. 按区域填写表单：
   ...
   - **任务类型及内容** —— 选择 **文本**（发送固定消息）或 **Agent**（向 TQ-Claw 提问并
     转发回复），然后填入具体内容。
...
在这里编辑定义 TQ-Claw 人设和行为的文件——SOUL.md、AGENTS.md、
HEARTBEAT.md 等——全部在浏览器中完成。

> **多智能体工作区：** 从 **v0.1.0** 开始，TQ-Claw 支持**多智能体工作区**功能。
> 您可以在同一个 TQ-Claw 实例中运行多个独立的智能体，每个智能体拥有独立的
...
在这里管理扩展 TQ-Claw 能力的技能（如读取 PDF、创建 Word 文档、获取新闻等）。
...
在这里配置 LLM 提供商并选择 TQ-Claw 使用的模型。TQ-Claw 同时支持云提供商（需要
...
本地提供商显示紫色的 **本地** 标签。
使用前需先安装后端依赖（`pip install 'tqclaw[llamacpp]'` 或
...
- 模型来自 Ollama 守护进程（不由 TQ-Claw 直接下载）
- 模型列表与 Ollama 自动同步
- 支持热门模型：`mistral:7b`、`qwen3:8b` 等

> 也可以通过 CLI 管理 Ollama 模型：`tqclaw models ollama-pull`、`tqclaw models ollama-list`、`tqclaw models ollama-remove`。详见 [CLI](./cli#ollama-模型)。

> ⚠️ **运行 TQ-Claw 前必须将上下文长度设为 32K 以上**
>
> 为了正常运行 TQ-Claw，必须将模型 context length 设置为 **32K 或更高**。注意，这会消耗较多的计算资源，请确保你的本地机器能够支持。
>
...
> LM Studio 默认不需要 API Key。模型必须在 LM Studio 中加载后才会在 TQ-Claw 中显示。
...
> **注意：** 云提供商 API Key 的有效性需要用户自行保证，TQ-Claw 不会验证。
>
...
在这里管理 TQ-Claw 的工具和技能在运行时需要的环境变量（如 `TAVILY_API_KEY`）。
...
> **注意：** 环境变量值的有效性需要用户自行保证，TQ-Claw 只负责存储和加载。
>
...
在聊天中直接问 TQ-Claw「最近用了多少 token？」或「帮我看看 token 消耗」，Agent 会调用 `get_token_usage` 工具并返回统计结果。

> 数据存储在 `~/.tqclaw/token_usage.json`，可通过 `TQCLAW_TOKEN_USAGE_FILE` 环境变量自定义文件名。详见 [配置 — 环境变量](./config#环境变量)。

...
In TQ-Claw, **heartbeat** means: on a fixed interval, ask TQ-Claw the
“questions” you wrote in a file, and optionally send the TQ-Claw’s reply to
**the channel where you last chatted**. Good for “regular check-ins, daily
digests, scheduled reminders” — TQ-Claw runs without you sending a
message.
...
1. You have a file **HEARTBEAT.md** (by default under the working dir
   `~/.tqclaw/`). Its content is **what to ask TQ-Claw each time** (one
   block of text; TQ-Claw sees it as one user message).
2. The system runs on your **interval** (e.g. every 30 minutes): read
   HEARTBEAT.md → send that as the user message → TQ-Claw replies.
3. **Whether the reply is sent to a channel** is controlled by **target** in
   config:
   - **main** — Run TQ-Claw only; do not send the reply anywhere (e.g. for
     local check-ins or logs).
   - **last** — Send the TQ-Claw’s reply to **the channel/session where you
     last talked to TQ-Claw** (e.g. if you last used DingTalk, the
...
Default path: `~/.tqclaw/HEARTBEAT.md`. Content = “what to ask each time.”
...
If you ran `tqclaw init` without `--defaults`, you were prompted to edit
...
`~/.tqclaw/config.json`), under `agents.defaults.heartbeat`:
...
> Need "send Good morning at 9am" or "every 2h ask todos and send to DingTalk"? Use [CLI](./cli) `tqclaw cron create` (cron jobs), not heartbeat.

...
TQ-Claw 所有配置和数据默认都在一个目录里，叫**工作目录**，默认是：

- **`~/.tqclaw`**（即你当前用户下的 `.tqclaw` 文件夹）

从 **v0.1.0** 开始，TQ-Claw 支持**多智能体工作区**。运行 `tqclaw init` 后会自动创建这个目录，新的结构如下：
...
**智能体工作区（`~/.tqclaw/workspaces/{agent_id}/`）**
...
> 会退回到通用的 "You are a helpful assistant" 提示。运行 `tqclaw init` 时会根据你选择的
...
如果你不想用 `~/.tqclaw`，可以通过环境变量改工作目录或某些文件的路径：

| 变量                      | 默认值             | 说明                                                                                                                                            |
| ------------------------- | ------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------- |
| `TQCLAW_WORKING_DIR`      | `~/.tqclaw`        | 工作目录；config、心跳、jobs、chats、skills、memory 都在这下面                                                                                  |
| `TQCLAW_SECRET_DIR`       | `~/.tqclaw.secret` | 敏感数据目录（工作目录的同级目录）；存放 `providers.json`（模型配置、API Key）和 `envs.json`（环境变量）。Docker 中默认为 `/app/working.secret` |
| `TQCLAW_CONFIG_FILE`      | `config.json`      | 配置文件名（相对工作目录）                                                                                                                      |
| `TQCLAW_HEARTBEAT_FILE`   | `HEARTBEAT.md`     | 心跳问题文件名（相对工作目录）                                                                                                                  |
| `TQCLAW_JOBS_FILE`        | `jobs.json`        | 定时任务文件名（相对工作目录）                                                                                                                  |
| `TQCLAW_CHATS_FILE`       | `chats.json`       | 会话列表文件名（相对工作目录）                                                                                                                  |
| `TQCLAW_TOKEN_USAGE_FILE` | `token_usage.json` | Token 消耗记录文件名（相对工作目录）                                                                                                            |

| `TQCLAW_LOG_LEVEL` | `info` | 日志级别（`debug`、`info`、`warning`、`error`、`critical`） |
| `TQCLAW_MEMORY_COMPACT_THRESHOLD` | `100000` | 触发记忆压缩的字符阈值 |
| `TQCLAW_MEMORY_COMPACT_KEEP_RECENT` | `3` | 压缩后保留的最近消息数 |
| `TQCLAW_MEMORY_COMPACT_RATIO` | `0.7` | 触发压缩的阈值比例（相对于上下文窗口大小） |
| `TQCLAW_CONSOLE_STATIC_DIR` | _（自动检测）_ | 控制台前端静态文件路径 |

例如在 Linux/macOS 里临时换工作目录：

```bash
export TQCLAW_WORKING_DIR=/home/me/my_tqclaw
tqclaw app
```

这样 config、HEARTBEAT、jobs、memory 等都会在 `/home/me/my_tqclaw` 下读写。
...
1. **全局配置** - `~/.tqclaw/config.json`（提供商、环境变量、智能体列表）
2. **智能体配置** - `~/.tqclaw/workspaces/{agent_id}/agent.json`（每个智能体的独立配置）
...
| `media_dir`          | string | `~/.tqclaw/media` | 接收到的媒体文件存放目录 |
...
| `agents.installed_md_files_language` | string \| null | `null` | 记录当前已安装的 MD 文件语言；由 `tqclaw init` 自动管理 |
...
| 字段   | 类型           | 默认值 | 说明                         |
| ------ | -------------- | ------ | ---------------------------- |
| `host` | string \| null | `null` | 上次 `tqclaw app` 绑定的主机 |
| `port` | int \| null    | `null` | 上次 `tqclaw app` 绑定的端口 |

每次运行 `tqclaw app` 时会自动保存。其他 CLI 子命令（如 `tqclaw cron`）会读取这个地址来发送请求。
...
TQ-Claw 需要 LLM 提供商才能运行。有三种设置方式：

- **`tqclaw init`** — 交互式向导，最简单
- **控制台 UI** — 在设置页面点选
...
> **提示：** 运行 `tqclaw init` 跟着提示走就行——它会列出每个提供商的可用模型让你直接选。
>
> **注意：** API Key 和 Base URL 的有效性需要用户自行保证。TQ-Claw 不会验证你填入的密钥是否正确或是否有余额，请确保所选提供商和模型可正常访问。
...
- **`tqclaw init`** — 初始化时会问 "Configure environment variables?"
- **控制台 UI** — 在设置页面编辑
- **API** — `GET/PUT/DELETE /envs`

设置好的变量会在应用启动时自动加载，所有工具和子进程都可以通过 `os.environ` 读取。

> **注意：** 环境变量的值（如第三方 API Key）的有效性需要用户自行保证。TQ-Claw 只负责存储和注入，不会校验其正确性。
...
| 目录                           | 说明                                                             |
| ------------------------------ | ---------------------------------------------------------------- |
| 内置（源码中）                 | 随 TQ-Claw 一起发布——docx、pdf、pptx、xlsx、news、email、cron 等 |
| `~/.tqclaw/customized_skills/` | 用户自己创建的技能                                               |
| `~/.tqclaw/active_skills/`     | 当前激活的技能（从内置 + 自定义同步过来的）                      |
...
- `tqclaw init`（初始化时选择 all / none / custom）
- `tqclaw skills config`（交互式开关）
- API 接口（`/skills/...`）
...
TQ-Claw 拥有跨对话的持久记忆能力：自动压缩上下文，并将关键信息写入 Markdown 文件长期保存。详细说明请看 [记忆](./memory.zh.md)。

记忆文件存储在两个位置：

| 文件/目录                        | 说明                                       |
| -------------------------------- | ------------------------------------------ |
| `~/.tqclaw/MEMORY.md`            | 长期有效的关键信息（决策、偏好、持久事实） |
| `~/.tqclaw/memory/YYYY-MM-DD.md` | 每日日志（日常笔记、运行上下文、自动摘要） |

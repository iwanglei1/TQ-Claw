...
| `/dump_history` | ⚡ 否    | 💾 保存       | -           | 📁 历史导出文件路径      |
| `/load_history` | ⚡ 否    | 📥 恢复       | -           | ✅ 历史加载结果          |
...
在对话中发送 `/daemon <子命令>` 或在终端执行 `tqclaw daemon <子命令>`，可查看状态、最近日志、版本等，无需经过 Agent。支持短名（如
...
| `/daemon logs` 或 `/daemon logs 50` | 查看最近 N 行控制台日志（默认 100 行，来自工作目录下 `tqclaw.log`）    |

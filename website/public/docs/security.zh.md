...
TQ-Claw 内置了安全功能，保护你的 Agent 免受恶意输入和不安全技能的影响。这些功能在控制台 **设置 → 安全** 中配置，也可以通过 `config.json` 进行设置。
...
在控制台（**设置 → 安全 → 技能扫描器 → 扫描模式**）或通过环境变量 `TQCLAW_SKILL_SCAN_MODE`（`block`、`warn` 或 `off`）设置。环境变量优先于配置文件。
...
扫描器使用 `src/tqclaw/security/skill_scanner/rules/signatures/` 中的 YAML 规则文件。你可以通过 YAML 策略文件自定义扫描策略：
...
TQ-Claw 支持可选的 Web 登录认证，保护控制台免受未授权访问。认证**默认关闭**，需要通过 `TQCLAW_AUTH_ENABLED` 环境变量显式启用。关闭时，TQ-Claw 的行为与默认配置完全一致 — 无登录页面，无令牌检查。

### 工作原理

1. 设置 `TQCLAW_AUTH_ENABLED=true` 并启动 TQ-Claw。
...
6. 来自**本地**（`127.0.0.1` / `::1`）的请求自动跳过认证，因此 CLI 命令（`tqclaw app`、`tqclaw chat` 等）无需令牌即可正常工作。
...
然后通过 `--env-file .env` 传递给 Docker，或在运行 `tqclaw app` 前在 shell 中 source 该文件。

### 关闭认证

移除或取消环境变量并重启 TQ-Claw：
...
```bash
docker exec -it <容器名> tqclaw auth reset-password
```

> **替代方案**：也可以删除 `SECRET_DIR`（默认 `~/.tqclaw/.secret/`）下的 `auth.json` 文件并重启 TQ-Claw。这会完全移除已注册的账户，下次访问时可以重新注册。

...
### TQ-Claw vs OpenClaw: Feature Comparison

Please check the [Comparison](/docs/comparison) page for detailed feature comparison.

### How to install TQ-Claw

TQ-Claw supports multiple installation methods. See
[Quick Start](https://tqclaw.agentscope.io/docs/quickstart) for details:
...
### How to update TQ-Claw

To update TQ-Claw, use the method matching your installation type:
...
After upgrading, restart the service with `tqclaw app`.

### How to initialize and start TQ-Claw service
...
[Quick Start](https://tqclaw.agentscope.io/docs/quickstart).

### Open-source repository

TQ-Claw is open source. Official repository:
`https://github.com/agentscope-ai/TQ-Claw`

### Where to check latest version upgrade details

You can check version changes in TQ-Claw GitHub
[Releases](https://github.com/agentscope-ai/TQ-Claw/releases).

### How to configure models

In Console, go to **Settings -> Models** to configure. See the
[Models](https://tqclaw.agentscope.io/docs/models) doc for details:
...
You can also use `tqclaw models` CLI commands for configuration, download, and
switching. See
[CLI -> Models and environment variables -> tqclaw models](https://tqclaw.agentscope.io/docs/cli#tqclaw-models).

### When using models deployed with Ollama / LM Studio, why can't TQ-Claw complete multi-turn interactions, complex tool calls, or remember earlier instructions?

In most cases, this is not a TQ-Claw bug. The root cause is usually that the
model's context length is configured too small.

When you deploy a local model with Ollama or LM Studio, if the model's
`context length` is too low, TQ-Claw may show problems such as:
...
- Before running TQ-Claw, set the model's `context length` to **at least 32K**
- For more complex tasks, frequent tool calls, or longer conversations, you
  may need a value **higher than 32K**

> ⚠️ **Before running TQ-Claw, you must set the context length to 32K or higher**
>
> For local models deployed with Ollama or LM Studio, TQ-Claw typically needs a
...
The easiest way to create a cron job is to talk to TQ-Claw in the channel where you want the results. For example, say: “Create a scheduled task that reminds me to drink water every five minutes.” You can then see the enabled job in Console.

If a scheduled task does not run as expected, try the following:

1. Confirm that the TQ-Claw service is running.
...
6. To verify that the task was created and can run, click **Execute Now**. If it works, you should see the reply in the target channel. You can also ask TQ-Claw: “Trigger the ‘drink water reminder’ task I just created.”
...
[Skills](https://tqclaw.agentscope.io/docs/skills).

### How to configure MCP

Go to **Agent -> MCP** in Console. You can enable/disable/delete/create MCP
clients there. See [MCP](https://tqclaw.agentscope.io/docs/mcp).
...
TQ-Claw supports API keys obtained via DashScope Coding Plan. If it still fails,
please check:
...
[issue](https://github.com/agentscope-ai/TQ-Claw/issues) in the TQ-Claw GitHub

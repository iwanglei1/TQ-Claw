...
- **`~/.tqclaw`** (the `.tqclaw` folder under your home directory)

Starting from **v0.1.0**, TQ-Claw supports **multi-agent workspace**. When you run `tqclaw init`, the new structure looks like:
...
**Agent Workspace (`~/.tqclaw/workspaces/{agent_id}/`)**
...
> generic "You are a helpful assistant" prompt. Run `tqclaw init` to auto-copy
...
| `TQCLAW_CHATS_FILE`       | `chats.json`       | Chats file name (relative to working dir)                                                                                                                                               |
| `TQCLAW_TOKEN_USAGE_FILE` | `token_usage.json` | Token usage record file name (relative to working dir)                                                                                                                                  |

| `TQCLAW_LOG_LEVEL` | `info` | Log level for the app (`debug`, `info`, `warning`, `error`, `critical`) |
| `TQCLAW_MEMORY_COMPACT_THRESHOLD` | `100000` | Character threshold to trigger memory compaction |
| `TQCLAW_MEMORY_COMPACT_KEEP_RECENT` | `3` | Number of recent messages kept after compaction |
| `TQCLAW_MEMORY_COMPACT_RATIO` | `0.7` | Threshold ratio for triggering compaction (relative to context window) |
| `TQCLAW_CONSOLE_STATIC_DIR` | _(auto-detect)_ | Path to the console front-end static files |
...
This is auto-saved every time you run `tqclaw app`. Other CLI subcommands
(like `tqclaw cron`) use this to know where to send requests.
...
TQ-Claw needs an LLM provider to work. You can set it up in three ways:

- **`tqclaw init`** — interactive wizard, the easiest way
...
> **Tip:** Run `tqclaw init` and follow the prompts — it will list available
...
> TQ-Claw does not verify whether the key is correct or has sufficient quota —
...
- **`tqclaw init`** — prompts "Configure environment variables?" during setup
...
> keys) are valid. TQ-Claw only stores and injects them — it does not verify
> correctness.
...
- `tqclaw init` (choose all / none / custom during setup)
- `tqclaw skills config` (interactive toggle)
...
TQ-Claw has persistent cross-conversation memory: it automatically compresses context and saves key information to Markdown files for long-term retention. See [Memory](./memory.en.md) for full details.
...
- Everything lives under **`~/.tqclaw`** by default; override with
  `TQCLAW_WORKING_DIR` (and related env vars) if needed.
- From **v0.1.0**, configuration is split into:
  - **Global config** (`~/.tqclaw/config.json`) — providers, environment variables, agent list
  - **Agent config** (`~/.tqclaw/workspaces/{agent_id}/agent.json`) — per-agent settings
...
- LLM providers are globally configured via `tqclaw init` or the console UI.

...
- **方式六 — 阿里云 ECS**：在阿里云上一键部署 TQ-Claw，无需本地安装。

> 📖 阅读前请先了解 [项目介绍](./intro)，完成安装与启动后可查看 [控制台](./console)。

> 💡 **安装并启动后**：在配置频道之前，可先打开 [控制台](./console)（浏览器访问 `http://127.0.0.1:8088/`）与 TQCLAW 对话、配置 Agent；要在钉钉、飞书、QQ 等 app 里对话时，再前往 [频道配置](./channels) 接入频道。
...
然后打开新终端（安装脚本会自动将 TQ-Claw 加入 PATH）。
...
升级只需重新运行安装命令。卸载请运行 `tqclaw uninstall`。

### 步骤二：初始化

在工作目录（默认 `~/.tqclaw`）下生成 `config.json` 与 `HEARTBEAT.md`。两种方式：
...
若已有配置想覆盖，可使用 `tqclaw init --force`（会提示确认）。
...
服务默认监听 `127.0.0.1:8088`。若已配置频道，TQ-Claw 会在对应 app 内回复；若尚未配置，也可先完成本节再前往频道配置。
...
`source .venv/bin/activate`，Windows 下 `.venv\Scripts\Activate.ps1`）。安装后会提供 `tqclaw` 命令。
...
1. **下载安装包**
   前往 [GitHub Releases](https://github.com/agentscope-ai/TQ-Claw/releases) 下载对应系统的版本：
...
2. **安装并启动**
   ...
   - **macOS**: 解压 `.zip` 得到 `TQ-Claw.app`，首次需右键选择"打开"以绕过系统安全限制
...
若不想在本地安装 Python，可通过魔搭创空间将 TQ-Claw 部署到云端运行：

1. 先前往 [魔搭](https://modelscope.cn/register?back=%2Fhome) 注册并登录；
2. 打开 [TQ-Claw 创空间](https://modelscope.cn/studios/fork?target=AgentScope/TQ-Claw)，一键配置即可使用。

**重要**：使用创空间请将空间设为 **非公开**，否则你的 TQ-Claw 可能被他人操纵。

---

## 方式五：Docker

镜像在 **Docker Hub**（`agentscope/tqclaw`）。镜像 tag：`latest`（稳定版）；`pre`（PyPI 预发布版）。国内用户也可选用阿里云 ACR：`agentscope-registry.ap-southeast-1.cr.aliyuncs.com/agentscope/tqclaw`（tag 相同）。
...
然后在浏览器打开 **http://127.0.0.1:8088/** 进入控制台。配置、记忆与 Skills 保存在 `tqclaw-data` 卷中；模型配置与 API Key 保存在 `tqclaw-secrets` 卷中。传入 API Key 可在 `docker run` 时加 `-e DASHSCOPE_API_KEY=xxx` 或 `--env-file .env`。

---

## 方式六：部署到阿里云 ECS

若希望将 TQ-Claw 部署在阿里云上，可使用阿里云 ECS 一键部署：

1. 打开 [TQ-Claw 阿里云 ECS 部署链接](https://computenest.console.aliyun.com/service/instance/create/cn-hangzhou?type=user&ServiceId=service-1ed84201799f40879884)，按页面提示填写部署参数；
2. 参数配置完成后确认费用并创建实例，部署完成后即可获取访问地址并使用服务。

详细步骤与说明请参考 [阿里云开发者社区：TQ-Claw 3 分钟部署你的 AI 助理](https://developer.aliyun.com/article/1713682)。
...
- **想和 TQCLAW 对话** → 去 [频道配置](./channels) 接一个频道（推荐先接钉钉或飞书），按文档申请应用、填 config，保存后即可在对应 app 里发消息试。

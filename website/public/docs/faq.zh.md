...
### TQ-Claw 与 OpenClaw 的功能对比

请查看 [对比](/docs/comparison) 页面了解详细的功能对比。

### TQ-Claw如何安装

TQ-Claw 支持多种安装方式，详情请见文档 [快速开始](https://tqclaw.agentscope.io/docs/quickstart)：
...
要更新 TQ-Claw 到最新版本，可根据你的安装方式选择对应方法：
...
控制台默认地址为 `http://127.0.0.1:8088/`，使用默认配置快速初始化后，可以进入控制台快捷自定义相关内容。详情请见[快速开始](https://tqclaw.agentscope.io/docs/quickstart)。
...
具体版本变更可在 TQ-Claw GitHub 仓库 [Releases](https://github.com/agentscope-ai/TQ-Claw/releases) 中查看。

### 如何配置模型

在控制台进入 **设置 → 模型** 中进行配置，详情请见文档 [模型](https://tqclaw.agentscope.io/docs/models)：

- 云端模型：填写提供商 API Key（如 ModelScope、DashScope 或自定义提供商），再选择活跃模型。
- 本地模型：支持 `llama.cpp`、`MLX` 和 Ollama。下载后可在同页选择活跃模型。

命令行也可使用 `tqclaw models` 系列命令完成配置、下载和切换，详情请见文档 [CLI → 模型与环境变量 → tqclaw models](https://tqclaw.agentscope.io/docs/cli#tqclaw-models)。

### 使用 Ollama / LM Studio 部署的模型时，为什么 TQ-Claw 无法完成多轮交互、复杂工具调用，或记不住之前的指令？

这类问题通常不是 TQ-Claw 本身异常，而是**模型上下文长度配置过小**导致的。

当你使用 Ollama 或 LM Studio 部署本地模型时，如果模型的 `context length` 设置太低，TQ-Claw 在以下场景中就可能表现异常：
...
- 运行 TQ-Claw 前，请将模型的 `context length` 设置为**至少 32K**
- 如果任务较复杂、工具调用较多或对话轮次较长，实际可能需要设置到**高于 32K**

> ⚠️ **运行 TQ-Claw 前必须将上下文长度设为 32K 以上**
>
> 对于 Ollama 和 LM Studio 部署的本地模型，如果要让 TQ-Claw 正常完成多轮交互、复杂工具调用和长上下文任务，通常必须提供 **32K 或更高** 的上下文长度；在更复杂的场景下，可能还需要进一步提高。
>
...
最方便的定时任务创建方式是，在你想要获取定时任务返回结果的频道，与TQ-Claw对话，让TQ-Claw帮你创建一个定时任务。例如，可以直接与TQ-Claw对话：“帮我创建一个定时任务，每隔五分钟提醒我喝水。”之后可以在控制台中看到状态为已启用的定时任务。

如果定时任务没有正常启动，可以按照以下几个步骤排查：

1. 首先确认 TQ-Claw 服务是在正常运行中的。
...
6. 排查结束后，如果想确认一下定时任务是否创建成功，且能成功触发，可以点击 **立即执行**，若成功创建，则可在对应频道收到回复。或者也可以直接与 TQ-Claw 对话：“帮我触发一下刚刚创建的提醒喝水定时任务”。

   ![exec](https://img.alicdn.com/imgextra/i4/O1CN017tycJh1ZPhO5XMuAu_!!6000000003187-2-tps-3020-762.png)

### 如何管理Skill

进入控制台 **智能体 → 技能**，可以启用/禁用技能、创建自定义技能、以及从 Skills Hub 中导入技能。详情请见文档 [Skills](https://tqclaw.agentscope.io/docs/skills)。

### 如何配置MCP

进入控制台 **智能体 → MCP**，进行 MCP 客户端的启用/禁用/删除/创建，详情请见文档 [MCP](https://tqclaw.agentscope.io/docs/mcp)。
...
TQ-Claw 支持百炼 Coding Plan 获取的 API key。如果仍报错，请重点检查：
...
为了加快修复与排查，共建良好社区生态，建议遇到报错时，首选在 TQ-Claw 的 GitHub 仓库中提 [issue](https://github.com/agentscope-ai/TQ-Claw/issues)，请附上完整报错信息，并上传错误详情文件。
...
请将后面的`/var/folders/.../tqclaw_query_error_qzbx1mv1.json`文件一并上传，同时提供你当前的模型提供商、模型名和 TQ-Claw 的具体版本。

...
`~/.tqclaw/active_skills/`): any subdirectory containing a `SKILL.md` is loaded as a
...
| ---------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------- |
| **cron**                     | Scheduled jobs. Create, list, pause, resume, or delete jobs via `tqclaw cron` or Console **Control → Cron Jobs**; run on a schedule and send results to a channel.           | Built-in                                                       |
...
`~/.tqclaw/active_skills/cron/`. It provides “run on a schedule and send results to a
channel.” You manage jobs with the [CLI](./cli) (`tqclaw cron`) or in the
Console under **Control → Cron Jobs**; no need to edit skill files.

Common operations:

- Create a job: `tqclaw cron create --type agent --name "xxx" --cron "0 9 * * *" ...`
- List jobs: `tqclaw cron list`
- Check state: `tqclaw cron state <job_id>`
...
1. Create a directory under `~/.tqclaw/customized_skills/`, e.g. `my_skill`.
...
On startup the app merges built-in skills with custom skills from `~/.tqclaw/customized_skills/` into `~/.tqclaw/active_skills/`; custom skills take priority when names collide. Your custom directories are never overwritten; built-in skills are only copied to `active_skills` when missing.

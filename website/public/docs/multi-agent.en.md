...
TQ-Claw supports **multi-agent workspace**, allowing you to run multiple independent AI agents in a single TQ-Claw instance, each with its own configuration, memory, skills, and conversation history.
...
Simply put, **multi-agent** lets you run multiple "personas" in one TQ-Claw, where each persona:
...
You might use TQ-Claw across multiple platforms:
...
After starting TQ-Claw, you'll see the **Agent Selector** in the **top-right corner** of the console:
...
**Note**: After deletion, the workspace directory is retained (to prevent accidental data loss). To completely remove it, manually delete the `~/.tqclaw/workspaces/{agent_id}` directory.
...
If you previously used TQ-Claw **v0.0.x**, upgrading to **v0.1.0** will **automatically migrate**:
...
2. **Verify Migration**
   ...
   - After starting TQ-Claw, check the agent list in console
...
tqclaw channels list --agent-id abc123
tqclaw cron list --agent-id abc123
tqclaw skills list --agent-id abc123
...
- `tqclaw channels` - channel management
- `tqclaw cron` - cron jobs
- `tqclaw daemon` - runtime status
- `tqclaw chats` - chat management
- `tqclaw skills` - skill management

**Commands NOT Supporting `--agent-id`** (global operations):

- `tqclaw init` - initialization
- `tqclaw providers` - model providers
- `tqclaw models` - model configuration
- `tqclaw env` - environment variables

...
You need to configure a model before chatting with TQ-Claw. You can do this under **Console → Settings → Models**.

![Console models](https://img.alicdn.com/imgextra/i1/O1CN01zHAE1Z26w6jXl2xbr_!!6000000007725-2-tps-3802-1968.png)

TQ-Claw supports multiple LLM providers: **cloud providers** (require API Key, including Google Gemini), **local providers** (llama.cpp / MLX), **Ollama provider**, **LM Studio provider**, and you can add **custom providers**. This page explains how to configure each type.
...
tqclaw models config-key gemini

# Set Gemini as the active LLM
tqclaw models set-llm
```

> **Tip:** Gemini models with thinking capabilities (e.g. Gemini 3.1 Pro, Gemini 2.5 Pro, Gemini 2.5 Flash) support extended reasoning. TQ-Claw automatically handles thinking blocks and thought signatures from these models.
...
- Install the matching backend in the same environment as TQ-Claw:
  - llama.cpp: `pip install 'tqclaw[llamacpp]'`
  - MLX: `pip install 'tqclaw[mlx]'`
...
The Ollama provider uses the **Ollama daemon** installed on your machine. Models are managed by Ollama; TQ-Claw does not download them directly, and the list syncs with Ollama.

**Prerequisites:**

- Install Ollama from [ollama.com](https://ollama.com).
- Install Ollama support in TQ-Claw’s environment: `pip install 'tqclaw[ollama]'`.
...
> If you see `Ollama SDK not installed. Install with: pip install 'tqclaw[ollama]'`, make sure Ollama is installed from ollama.com and you’ve run `pip install 'tqclaw[ollama]'` in TQ-Claw’s environment. To remove a model, click **Models** on the Ollama card, then the **trash icon** next to the model and confirm.
>
> ![delete](https://img.alicdn.com/imgextra/i1/O1CN01OvNNu21shXVzD14go_!!6000000005798-2-tps-3802-1968.png)
>
> **Docker users:** If TQ-Claw runs inside a Docker container, `localhost` refers to the container — not your host machine. Change the Ollama Base URL to `http://host.docker.internal:11434` (and add `--add-host=host.docker.internal:host-gateway` to your `docker run` command). See the [Docker section in the README](https://github.com/agentscope-ai/TQ-Claw#using-docker) for details.

## LM Studio provider

The LM Studio provider connects to the **LM Studio** desktop application's built-in OpenAI-compatible server. Models are managed in the LM Studio GUI; TQ-Claw discovers loaded models automatically via the `/v1/models` endpoint.
...
> **Tip:** LM Studio does not require an API key by default. If you have enabled authentication in LM Studio, enter the key in the **API Key** field. Models must be loaded in LM Studio's GUI before they appear in TQ-Claw.
>
> **Important — Context Length:** LM Studio loads models with a small default context length (often 2048 or 4096 tokens). TQ-Claw's system prompt (AGENTS.md + SOUL.md + PROFILE.md) can easily exceed this limit, causing an error like _"The number of tokens to keep from the initial prompt is greater than the context length"_. To fix this, **unload the model in LM Studio and reload it with a larger context length** (16384 or above is recommended). You can do this in the LM Studio GUI (Model Settings → Context Length) or via the CLI: `lms unload --all && lms load <model> -c 16384`.
>

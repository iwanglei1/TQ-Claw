...
TQ-Claw divides the context into three zones:
...
- [MemoryCompactionHook](https://github.com/agentscope-ai/TQ-Claw/blob/main/src/tqclaw/agents/hooks/memory_compaction.py)
...
When the context approaches its limit, TQ-Claw automatically triggers compaction, condensing old conversations into a structured summary.
...
Configuration is located in `~/.tqclaw/config.json` under `agents.running`:

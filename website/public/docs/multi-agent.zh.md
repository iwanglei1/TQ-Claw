...
TQ-Claw 支持**多智能体工作区**，允许您在同一个 TQ-Claw 实例中运行多个独立的 AI 智能体，每个智能体拥有自己的配置、记忆、技能和对话历史。
...
简单来说，**多智能体**就是让您可以在一个 TQ-Claw 中运行多个"分身"，每个分身：
...
启动 TQ-Claw 后，在控制台**右上角**可以看到**智能体切换器**：
...
**注意**：删除后工作区目录会保留（防止误删数据），如需彻底清理，请手动删除 `~/.tqclaw/workspaces/{agent_id}` 目录。
...
如果您之前使用 TQ-Claw **v0.0.x**，升级到 **v0.1.0** 时会**自动迁移**：
...
```bash
# 备份特定智能体
cp -r ~/.tqclaw/workspaces/abc123 ~/backups/agent-abc123-$(date +%Y%m%d)

# 备份所有智能体
cp -r ~/.tqclaw/workspaces ~/backups/workspaces-$(date +%Y%m%d)
```

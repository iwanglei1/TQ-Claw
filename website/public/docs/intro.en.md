...
This page describes what TQ-Claw is, what it can do, and how to get started by
following the docs.
...
TQ-Claw is a **personal assistant** that runs in your own environment.
...
TQ-Claw is built by the [AgentScope team](https://github.com/agentscope-ai) on
...
## How do you use TQ-Claw?

You use TQ-Claw in two main ways:

1. **Chat in your messaging apps**
   Send messages in DingTalk, Feishu, QQ, Discord, or iMessage (Mac only); TQ-Claw replies
   in the same app and can look things up, manage todos, answer questions —
   whatever the enabled Skills support. One TQ-Claw instance can be connected to
   several apps; it replies in the channel where you last talked.

2. **Run on a schedule**
   Without sending a message each time, TQ-Claw can run at set times:
   - Send a fixed message to a channel (e.g. “Good morning” to DingTalk at 9am);
   - Ask TQ-Claw a question and send the answer to a channel (e.g. every 2 hours
     ask “What are my todos?” and post the reply to DingTalk);
   - Run a “check-in” or digest: ask TQ-Claw a block of questions you wrote and
     send the answer to the channel you last used.

After you install, connect at least one channel, and start the server, you can
chat with TQ-Claw in DingTalk, Feishu, QQ, etc. and use scheduled messages and check-ins;
...
- **Channel** — Where you talk to TQ-Claw (DingTalk, Feishu, QQ, Discord, iMessage, etc.).
  Configure each in [Channels](./channels).
- **Heartbeat** — On a fixed interval, ask TQ-Claw a block of text you wrote and
...
- **Cron jobs** — Scheduled tasks (send X at 9am, ask Y every 2h, etc.), managed
  via [CLI](./cli) or API.
- **Agent/Workspace** — Starting from **v0.1.0**, TQ-Claw supports multi-agent workspace,

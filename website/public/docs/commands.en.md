...
In chat, send `/daemon <subcommand>` or use short names (e.g., `/status` is equivalent to `/daemon status`). From the terminal, run `tqclaw daemon <subcommand>`. These run without going through the Agent.
...
| `/daemon logs` or `/daemon logs 50` | View last N lines of console log (default 100, from `tqclaw.log` in working directory)         |

From the terminal:

```bash
tqclaw daemon status

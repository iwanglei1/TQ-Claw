# TQ-Claw Desktop Application Guide
...
This guide explains how to install and use the TQ-Claw Desktop application on Windows and macOS.
...
1. **Download the installer**
   Download `TQ-Claw-Setup-<version>.exe` from the [Release page][releases]

2. **Run the installer**
   Double-click the `.exe` file and follow the installation wizard
   - Default installation location: `C:\Users\<your-username>\AppData\Local\TQ-Claw`
...
#### **TQ-Claw Desktop** (Recommended for daily use)
...
- **How to Launch**: Double-click the "TQ-Claw Desktop" icon on desktop or Start Menu
- **Technical Note**: Uses VBScript launcher, runs Python process in background

#### **TQ-Claw Desktop (Debug)** (Debug Mode)
...
- **How to Launch**: Double-click "TQ-Claw Desktop (Debug)" icon in Start Menu
- **Log Contents**:
...
A: Use "TQ-Claw Desktop (Debug)" mode to view terminal output for error messages

**Q: How to uninstall?**
A: Go to Windows Settings → Apps → Installed apps → Find "TQ-Claw Desktop" → Uninstall
...
1. **Download the archive**
   Download `TQ-Claw-<version>-macOS.zip` from the [Release page][releases]

2. **Extract**
   Double-click the `.zip` file to extract and get `TQ-Claw.app`

3. **Move to Applications folder (Optional)**
   Drag `TQ-Claw.app` to the `/Applications` folder

### First Launch: Bypassing System Security Restrictions

#### Why manual trust is needed?

TQ-Claw is **not Apple Developer-signed or notarized**, so macOS Gatekeeper will block it by default.
...
1. **Right-click** (or Control + click) on `TQ-Claw.app`
...
2. Scroll down to find a message like:
   _"'TQ-Claw' was blocked from use because it cannot verify the developer"_
...
- Double-click `TQ-Claw.app` to launch
- The app runs in the background and opens a browser window
- Logs are written to: `~/.tqclaw/desktop.log`
...
cd /Applications  # or wherever your TQ-Claw.app is located

# Set environment variables and launch
APP_ENV="$(pwd)/TQ-Claw.app/Contents/Resources/env"
PYTHONPATH= PYTHONHOME="$APP_ENV" "$APP_ENV/bin/python" -m tqclaw desktop
```
...
1. Check the `~/.tqclaw/desktop.log` file for errors
...
**Q: How to uninstall?**
A: Drag `TQ-Claw.app` to the Trash, then delete the `~/.tqclaw` configuration folder
...
- **GitHub Issues**: [Submit an issue](https://github.com/agentscope-ai/TQ-Claw/issues)
- **Packaging documentation**: `scripts/pack/README.md` - Technical details and local build guide
- **Log locations**:
  - Windows: View in Debug mode terminal, or `%USERPROFILE%\.tqclaw\` directory
  - macOS: `~/.tqclaw/desktop.log`

...
The **Console** is TQ-Claw's built-in web interface. After running `tqclaw app`,
open `http://127.0.0.1:8088/` in your browser to enter the Console.

**In the Console, you can:**

- Chat with TQ-Claw in real time
- Enable/disable messaging channels
- View and manage all chat sessions
- Manage scheduled jobs
- Edit TQ-Claw's persona and behavior files
- Enable/disable skills to extend TQ-Claw's capabilities
- Manage MCP clients
...
This is where you talk to TQ-Claw. It is the default page when the Console
opens.
...
button (↑). TQ-Claw replies in real time.
...
Create and manage scheduled jobs that TQ-Claw runs automatically by time.

![Cron Jobs](https://img.alicdn.com/imgextra/i3/O1CN01JET1Aw1C9SAvXuIpk_!!6000000000038-2-tps-3822-2070.png)

**Create a new job:**

> If the cron job fails to be created, please refer to the **Troubleshooting Scheduled (Cron) Tasks** section in the [FAQ](https://tqclaw.agentscope.io/docs/faq) to identify the cause.

The **simplest way to create a cron job is to chat directly with TQ-Claw** and let it handle the creation for you. For example, if you want to receive a reminder to drink water on DingTalk, simply message TQ-Claw on DingTalk: "Help me create a cron job to remind me to drink water every 5 minutes." Once created, you can view the new task on the Cron Jobs page in the console.
...
2. Fill in each section:
   ...
   - **Task Type & Content** — **Text** (fixed message) or **Agent** (ask
     TQ-Claw and forward reply), then the content
...
Edit files that define TQ-Claw's persona and behavior, such as `SOUL.md`,
`AGENTS.md`, and `HEARTBEAT.md`, directly in the browser.

> **Multi-Agent Workspace:** Starting from **v0.1.0**, TQ-Claw supports
> **multi-agent workspace** functionality. You can run multiple independent
> agents in a single TQ-Claw instance, each with its own workspace, configuration,
...
Manage skills that extend TQ-Claw's capabilities (for example: PDF reading,
...
Configure LLM providers and choose the model TQ-Claw uses. TQ-Claw supports both
...
first (`pip install 'tqclaw[llamacpp]'` or `pip install 'tqclaw[mlx]'`).
...
- Models come from the Ollama daemon (not downloaded directly by TQ-Claw)
- Model list is auto-synced with Ollama
- Popular model examples: `mistral:7b`, `qwen3:8b`

> You can also manage Ollama models via CLI: `tqclaw models ollama-pull`,
> `tqclaw models ollama-list`, `tqclaw models ollama-remove`. See
> [CLI](./cli#ollama-models).

> ⚠️ **Before running TQ-Claw, you must set the context length to 32K or higher**
>
> To run TQ-Claw properly, you must set the model context length to
...
> in LM Studio before they appear in TQ-Claw.
...
> **Note:** Cloud API key validity is your responsibility. TQ-Claw does not
...
Manage runtime environment variables needed by TQ-Claw tools and skills
...
> **Note:** Variable validity is your responsibility. TQ-Claw only stores and
> loads values.
>
...
Ask TQ-Claw directly, e.g. "How many tokens have I used recently?" or "Show me token usage." The agent will call the `get_token_usage` tool and return the summary.

> Data is stored in `~/.tqclaw/token_usage.json`. You can override the filename with the `TQCLAW_TOKEN_USAGE_FILE` environment variable. See [Config — Environment Variables](./config#environment-variables).
...
| --------------------- | -------------------------------- | -------------------------------------------------------------- |
| Chat                  | Chat → Chat                      | Talk with TQ-Claw, manage sessions                               |

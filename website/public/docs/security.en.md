...
TQ-Claw includes built-in security features to protect your agent from malicious inputs and unsafe skills. These are configured in the Console under **Settings → Security**, or via `config.json`.
...
Set the mode in Console (**Settings → Security → Skill Scanner → Scanner Mode**) or via the environment variable `TQCLAW_SKILL_SCAN_MODE` (`block`, `warn`, or `off`). The environment variable takes precedence over the config file.
...
The scanner uses YAML rule files in `src/tqclaw/security/skill_scanner/rules/signatures/`. You can customize the scan policy via a YAML policy file:

```python
from tqclaw.security.skill_scanner import SkillScanner
from tqclaw.security.skill_scanner.scan_policy import ScanPolicy
...
TQ-Claw supports optional web login authentication to protect the Console from unauthorized access. Authentication is **disabled by default** and must be explicitly enabled via the `TQCLAW_AUTH_ENABLED` environment variable. When disabled, TQ-Claw behaves identically to the default configuration — no login page, no token checks.

### How it works

1. Set `TQCLAW_AUTH_ENABLED=true` and start TQ-Claw.
...
6. Requests from **localhost** (`127.0.0.1` / `::1`) bypass authentication entirely, so CLI commands (`tqclaw app`, `tqclaw chat`, etc.) continue to work without a token.
...
```bash
export TQCLAW_AUTH_ENABLED=true
tqclaw app
```
...
```bash
docker run -e TQCLAW_AUTH_ENABLED=true \
...
services:
  tqclaw:
    image: agentscope/tqclaw:latest
    ports:
      - "127.0.0.1:8088:8088"
    environment:
      - TQCLAW_AUTH_ENABLED=true
    volumes:
      - tqclaw-data:/app/working
      - tqclaw-secrets:/app/working.secret
```
...
Then pass it to Docker with `--env-file .env`, or source it in your shell before running `tqclaw app`.

### Disable authentication

Remove or unset the environment variable and restart TQ-Claw:

```bash
# Linux / macOS
unset TQCLAW_AUTH_ENABLED
tqclaw app

# Docker — simply remove the -e flag. The example below includes volumes for persistence.
docker run -p 127.0.0.1:8088:8088 -v tqclaw-data:/app/working -v tqclaw-secrets:/app/working.secret agentscope/tqclaw:latest
```
...
```bash
docker exec -it <container_name> tqclaw auth reset-password
```

> **Alternative**: You can also delete the `auth.json` file from `SECRET_DIR` (default `~/.tqclaw/.secret/`) and restart TQ-Claw. This removes the registered account entirely and allows you to re-register from scratch on the next visit.

...
感谢你对 TQ-Claw 的关注！TQ-Claw 是一个开源的**个人 AI 助手**，可以在你自己的环境中运行——无论是你的机器还是云端。它可以连接钉钉、飞书、QQ、Discord、iMessage 等聊天应用，支持定时任务和心跳机制，并通过 **Skills** 扩展其能力。我们热烈欢迎能让 TQ-Claw 对所有人更有用的贡献：无论是添加新的频道、新的模型提供商、Skill，改进文档，还是修复 bug。

**快速链接：** [GitHub](https://github.com/agentscope-ai/TQ-Claw) · [文档](https://tqclaw.agentscope.io/) · [许可证：Apache 2.0](LICENSE)
...
- **检查 [Open Issues](https://github.com/agentscope-ai/TQ-Claw/issues)** 以及任何 [Projects](https://github.com/agentscope-ai/TQ-Claw/projects) 或路线图标签。
...
TQ-Claw 设计为**可扩展的**：你可以添加模型、频道、Skills 等。以下是我们关心的主要贡献领域。

---

### 添加新模型 / 模型提供商

TQ-Claw 支持**多种模型后端**：云 API（如 DashScope、ModelScope）、**Ollama** 和本地后端（**llama.cpp**、**MLX**）。你可以通过两种方式贡献：
...
1. **提供商定义**（在 `src/tqclaw/providers/registry.py` 或等效位置）：
...
2. **聊天模型类**（如果 API 不兼容 OpenAI）：
   - 实现一个继承自 `agentscope.model.ChatModelBase` 的类（或适用时使用 TQ-Claw 的本地/远程包装器）。
   - 如果 agent 同时使用流式和非流式，则都要支持；如果使用了 tools API，则遵守 `tool_choice` 和 tools。
   - 在注册表的聊天模型映射中注册该类，以便运行时可以按名称解析它（参见 `src/tqclaw/providers/registry.py` 中的 `_CHAT_MODEL_MAP`）。
...
频道是 TQ-Claw 与**钉钉、飞书、QQ、Discord、iMessage** 等通信的方式。你可以添加新频道，以便 TQ-Claw 可以与你喜欢的 IM 或机器人平台配合使用。
...
- **实现：** 实现 **`BaseChannel` 的子类**（在 `src/tqclaw/app/channels/base.py` 中）：
...
- **发现：** 内置频道在 `src/tqclaw/app/channels/registry.py` 中注册。**自定义频道**从工作目录加载：放置一个模块（如 `custom_channels/telegram.py` 或包 `custom_channels/telegram/`），定义一个带有 `channel` 属性的 `BaseChannel` 子类。
- **CLI：** 用户使用以下命令安装/添加频道：
  - `tqclaw channels install <key>` — 创建模板或从 `--path` / `--url` 复制
  - `tqclaw channels add <key>` — 安装并添加到配置
  - `tqclaw channels remove <key>` — 从 `custom_channels/` 中删除自定义频道
  - `tqclaw channels config` — 交互式配置
...
**Skills** 定义了 TQ-Claw 可以做什么：cron、文件读取、PDF/Office、新闻、浏览器等。我们欢迎**广泛有用的**基础 skills（生产力、文档、通信、自动化），适合大多数用户。
...
- **位置：** 内置 skills 位于 `src/tqclaw/agents/skills/<skill_name>/` 下。应用程序将内置和用户的 **customized_skills**（来自工作目录）合并到 **active_skills** 中；除了在目录中放置有效的 `SKILL.md` 外，不需要额外的注册。
...
- **Skills Hub：** TQ-Claw 支持从社区 hub（如 ClawHub）导入 skills。如果你希望你的 skill 可以通过 hub 安装，请遵循相同的 `SKILL.md` + `references/`/`scripts/` 布局和 hub 的打包格式。
...
TQ-Claw 旨在在 **Windows**、**Linux** 和 **macOS** 上运行。欢迎改进特定平台支持的贡献。

- **兼容性修复：** 路径处理、行尾、shell 命令或在不同操作系统上行为不同的依赖项。例如：内存/向量栈的 Windows 兼容性，或在 Linux 和 macOS 上都能工作的安装脚本。
- **安装和运行：** 一行安装（`install.sh`）、`pip` 安装，以及 `tqclaw init` / `tqclaw app` 应该在每个支持的平台上工作（或有清晰的文档说明）。对给定操作系统上的安装或启动的修复很有价值。
...
- **MCP（模型上下文协议）：** TQ-Claw 支持运行时 **MCP 工具**发现和热插拔。贡献新的 MCP 服务器或工具（或关于如何附加它们的文档）可以帮助用户扩展 agent 而无需更改核心代码。
- **文档：** 对 [文档](https://tqclaw.agentscope.io/)（位于 `website/public/docs/` 下）和 README 的修复和改进始终受欢迎。
...
- **讨论：** [GitHub Discussions](https://github.com/agentscope-ai/TQ-Claw/discussions)
- **Bug 和功能：** [GitHub Issues](https://github.com/agentscope-ai/TQ-Claw/issues)
- **社区：** 钉钉群（见 [README](README_zh.md)）和 [Discord](https://discord.gg/eYMpfnkG8h)

感谢你为 TQ-Claw 贡献代码。你的工作帮助它成为每个人更好的助手。🐾

...
from tqclaw.cli import shutdown_cmd as shutdown_cmd_module
from tqclaw.cli.shutdown_cmd import (
...
def test_shutdown_command_stops_backend_and_frontend(monkeypatch) -> None:
    monkeypatch.setattr(
        "tqclaw.cli.shutdown_cmd._listening_pids_for_port",
        lambda _port: {1001},
    )
    monkeypatch.setattr(
        "tqclaw.cli.shutdown_cmd._find_frontend_dev_pids",
        lambda: {2002},
    )
    monkeypatch.setattr(
        "tqclaw.cli.shutdown_cmd._find_desktop_wrapper_pids",
        set,
    )
    monkeypatch.setattr(
        "tqclaw.cli.shutdown_cmd._find_windows_wrapper_ancestor_pids",
        lambda _pids: set(),
    )
    monkeypatch.setattr(
        "tqclaw.cli.shutdown_cmd._terminate_pid",
        lambda _pid: True,
    )
...
def test_shutdown_command_reports_nothing_found(monkeypatch) -> None:
    ...
    assert result.exit_code != 0
    assert "No running TQ-Claw" in result.output


def test_shutdown_command_stops_windows_wrapper_ancestors(monkeypatch) -> None:
    monkeypatch.setattr("tqclaw.cli.shutdown_cmd.sys.platform", "win32")
    monkeypatch.setattr(
        "tqclaw.cli.shutdown_cmd._listening_pids_for_port",
        lambda _port: {24692},
    )
...
def test_terminate_pid_force_kills_on_windows(monkeypatch) -> None:
    ...
    monkeypatch.setattr("tqclaw.cli.shutdown_cmd.sys.platform", "win32")
    monkeypatch.setattr(
        "tqclaw.cli.shutdown_cmd._pid_exists",
        lambda _pid: True,
    )
    monkeypatch.setattr(
        "tqclaw.cli.shutdown_cmd._terminate_process_tree_windows",
        lambda pid, force=False: calls.append((pid, force)),
    )
    monkeypatch.setattr(
        "tqclaw.cli.shutdown_cmd._wait_for_pid_exit",
...
def test_terminate_pid_uses_windows_fallback(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "tqclaw.cli.shutdown_cmd._force_terminate_windows_process",
...
def test_pid_exists_uses_windows_snapshot(monkeypatch) -> None:
    monkeypatch.setattr("tqclaw.cli.shutdown_cmd.sys.platform", "win32")
    monkeypatch.setattr(
        "tqclaw.cli.shutdown_cmd._windows_process_snapshot",
        lambda: {29104: (1, "tqclaw.exe", "tqclaw app")},
    )
...
def test_find_windows_wrapper_ancestor_pids(monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "tqclaw.cli.shutdown_cmd._windows_process_snapshot",
        lambda: {
            24692: (1052, "python.exe", "python -m uvicorn tqclaw.app"),
            1052: (900, "tqclaw.exe", ""),
...
def test_terminate_pid_force_kills_on_unix(monkeypatch) -> None:
    ...
    monkeypatch.setattr("tqclaw.cli.shutdown_cmd.sys.platform", "darwin")
    monkeypatch.setattr(
        "tqclaw.cli.shutdown_cmd._pid_exists",
        lambda _pid: True,
    )
    monkeypatch.setattr(
        "tqclaw.cli.shutdown_cmd._signal_process_tree_unix",

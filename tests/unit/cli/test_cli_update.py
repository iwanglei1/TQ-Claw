...
from tqclaw.__version__ import __version__
from tqclaw.cli.main import cli
from tqclaw.cli.update_cmd import (
    InstallInfo,
    RunningServiceInfo,
...
def _install_info(
    *,
    ...
    return InstallInfo(
        package_dir="/tmp/site-packages/tqclaw",
...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            {
                "url": "file:///Users/test/TQ-Claw",
                "dir_info": {"editable": True},
            },
            ("editable", "file:///Users/test/TQ-Claw"),
        ),
        (
            {
                "url": "https://github.com/agentscope-ai/TQ-Claw.git",
                "vcs_info": {"vcs": "git", "commit_id": "abc123"},
            },
            ("vcs", "https://github.com/agentscope-ai/TQ-Claw.git"),
        ),
        (
            {"url": "file:///tmp/tqclaw.whl"},
            ("local", "file:///tmp/tqclaw.whl"),
        ),
        (
            {"url": "https://example.com/tqclaw.whl"},
            ("direct-url", "https://example.com/tqclaw.whl"),
        ),
    ],
)
...
def test_detect_installation(
    ...
    from tqclaw.cli import update_cmd as update_cmd_module
...
def test_update_reports_up_to_date(monkeypatch) -> None:
    ...
    assert "TQ-Claw is already up to date." in result.output
...
def test_probe_service_ignores_proxy_env(monkeypatch) -> None:
    ...
    monkeypatch.setattr("tqclaw.cli.update_cmd.httpx.get", _fake_get)
...
def test_update_blocks_running_service(monkeypatch) -> None:
    ...
    assert result.exit_code != 0
    assert "Please stop it before running `tqclaw update`" in result.output
    assert (
        "without `--yes` to confirm a forced `tqclaw shutdown`" in result.output
    )
...
def test_update_can_cancel_forced_shutdown(monkeypatch) -> None:
    ...
    assert result.exit_code == 0
    assert (
        "forcibly terminate the current TQ-Claw backend/frontend "
...
def test_update_can_force_shutdown_running_service(
    ...
    def _fake_shutdown(
        ...
        class _Result:
            returncode = 0
            stdout = "Stopped TQ-Claw processes: 1234\n"
    ...
    assert result.exit_code == 0
    assert "Running `tqclaw shutdown` before updating..." in result.output
    assert "Stopped TQ-Claw processes: 1234" in result.output
    assert "Starting TQ-Claw update..." in result.output
...
def test_update_worker_waits_for_launcher_exit(
    ...
    monkeypatch.setattr(
        "tqclaw.cli.update_cmd._wait_for_process_exit",
...
def test_run_update_worker_detached_spawns_without_capture(
    ...
    monkeypatch.setattr(
        "tqclaw.cli.update_cmd._spawn_update_worker",
        _fake_spawn,
    )
...
def test_update_worker_foreground_streams_output_and_cleans_plan(
    ...
    assert return_code == 0
    assert "[tqclaw] Updating TQ-Claw 1.0.0 -> 9.9.9..." in captured.out
    assert "[tqclaw] Using installer: integration-test" in captured.out
    assert "installer: preparing" in captured.out
    assert "installer: done" in captured.out
    assert "[tqclaw] Update completed successfully." in captured.out
...
def test_update_worker_foreground_propagates_failure_exit_code(
    ...
    assert "[tqclaw] Update failed with exit code 7." in captured.out
    assert not plan_path.exists()

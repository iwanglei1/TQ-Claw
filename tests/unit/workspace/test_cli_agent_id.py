...
from tqclaw.cli.channels_cmd import channels_group
from tqclaw.cli.cron_cmd import cron_group
from tqclaw.cli.daemon_cmd import daemon_group
from tqclaw.cli.chats_cmd import chats_group
from tqclaw.cli.skills_cmd import skills_group
from tqclaw.config.config import AgentProfileConfig
...
def test_channels_list_default_agent(
    ...
    """Test tqclaw channels list uses default agent by default."""
    ...
    with patch("tqclaw.cli.channels_cmd.load_agent_config") as mock_load:
...
def test_channels_list_custom_agent(
    ...
    """Test tqclaw channels list with custom agent_id."""
...
def test_cron_list_with_agent_id():
    """Test tqclaw cron list with --agent-id."""
    runner = CliRunner()

    with patch("tqclaw.cli.cron_cmd.client") as mock_client:
...
def test_daemon_status_default_agent():
    """Test tqclaw daemon status defaults to 'default' agent."""
    runner = CliRunner()

    with patch("tqclaw.cli.daemon_cmd.run_daemon_status") as mock_status:
        with patch("tqclaw.cli.daemon_cmd._get_agent_workspace") as mock_ws:
...
def test_daemon_status_custom_agent():
    """Test tqclaw daemon status with custom agent."""
    runner = CliRunner()
...
def test_skills_list_default_agent():
    """Test tqclaw skills list defaults to 'default' agent."""
    runner = CliRunner()

    with patch(
        "tqclaw.cli.skills_cmd._get_agent_workspace",
    ) as mock_ws:
        with patch("tqclaw.cli.skills_cmd.SkillService") as mock_service:
            mock_ws.return_value = "/tmp/default"
...
def test_skills_list_custom_agent():
    """Test tqclaw skills list with custom agent."""
    runner = CliRunner()
...
def test_chats_list_with_agent_id():
    """Test tqclaw chats list with --agent-id."""
    runner = CliRunner()

    with patch("tqclaw.cli.chats_cmd.client") as mock_client:
        mock_response = MagicMock()

...
If you believe you've found a security issue in TQ-Claw, please report it privately.

## Reporting

Report vulnerabilities of the TQ-Claw repository:

If you discover a security issue in TQ-Claw, please report it to us through the [Alibaba Security Response Center (ASRC)](https://security.alibaba.com/).
...
7. **Environment** (Python version, OS, how TQ-Claw is run)
...
- Tested version details (TQ-Claw version and/or commit SHA).
- Reproducible PoC against latest `main` or latest released version.
- Demonstrated impact tied to TQ-Claw's documented trust boundaries (see below).
- For exposed-secret reports: proof the credential is TQ-Claw-owned or grants access to TQ-Claw-operated infrastructure/services.
...
- Reports that assume per-user multi-tenant authorization on a shared TQ-Claw instance/config.
...
## Alignment with `tqclaw init`

The security model and recommended baseline in this document are **aligned with** what users see and accept during **`tqclaw init`**. The init flow shows a security notice that covers: single-operator boundary; shared delegated authority when multiple people message the same instance; restricting channels and users (allowlists); using separate config/credentials and OS users or hosts per trust boundary; least privilege and sandboxing; keeping secrets out of the working directory and skill-accessible paths; and reviewing config and skills regularly. When updating either this document or the `SECURITY_WARNING` in `src/tqclaw/cli/init_cmd.py`, keep the **concepts and recommendations** consistent so operators get the same message in both places.

## Security & Trust

Security handling is owned by the TQ-Claw maintainers. For sensitive reports, use the private advisory or a private channel as above.

## Bug Bounties

TQ-Claw is a community open-source project. There is no bug bounty program and no budget for paid reports. Please still disclose responsibly so we can fix issues quickly. The best way to help the project right now is by sending PRs or clear, reproducible reports.

## Operator Trust Model

TQ-Claw does **not** model one instance as a multi-tenant, adversarial user boundary.

- Authenticated callers to the same TQ-Claw instance (same config, same channel workspace) are treated as **trusted operators** for that instance.
...
- **Recommended mode**: one user per machine/host (or per OS user), one TQ-Claw config for that user, and one or more agents/skills inside that instance.
- If multiple users need TQ-Claw, use one host/OS user (or VPS) per user, or strict isolation; sharing one instance by mutually untrusted users is not the recommended default.
- Skills run with the same privileges as the TQ-Claw process; only install and enable skills you trust.


## Trusted Skills Concept

Skills/extensions are part of TQ-Claw's **trusted computing base** for an instance.
...
- Public internet exposure of TQ-Claw when the docs recommend against it.
- Using TQ-Claw in ways that the docs recommend not to.
- Deployments where mutually untrusted/adversarial operators share one TQ-Claw instance and config (e.g. reports expecting per-operator isolation for sessions, history, or similar).
...
- Exposed secrets that are third-party or user-controlled credentials (not TQ-Claw-owned and not granting access to TQ-Claw-operated infrastructure) without demonstrated TQ-Claw impact.
- Scanner-only claims against stale or nonexistent paths, or without a working repro.

## Deployment Assumptions

TQ-Claw security guidance assumes:

- The host where TQ-Claw runs is within a trusted OS/admin boundary.
- Anyone who can modify the TQ-Claw working directory and config (including `config.json`) is effectively a trusted operator.
...
TQ-Claw's security model is **“personal assistant”** (one trusted operator, potentially many agents/skills), not “shared multi-tenant bus.”

- If multiple people can message the same tool-enabled TQ-Claw instance (e.g. a shared channel workspace), they can all steer that agent within its granted permissions.
...
The TQ-Claw working directory is treated as **trusted local operator state**.
...
- Keep secrets out of the working directory and skill-accessible paths; this is part of the baseline recommended in `tqclaw init`.

## Skills Trust Boundary

Skills are loaded and run **in-process** (or under the same trust boundary) as the TQ-Claw runtime and are treated as trusted code.

- Skills can execute with the same OS privileges as the TQ-Claw process.
...
For more operational and hardening guidance, see the [documentation](https://tqclaw.agentscope.io/) and any security-related docs linked from the repo.

## Runtime Requirements

- **Python**: TQ-Claw requires a supported Python version (see [README](README.md)). Use a version with current security updates.

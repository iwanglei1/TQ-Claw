...
- **Check [Open Issues](https://github.com/agentscope-ai/TQ-Claw/issues)** and any [Projects](https://github.com/agentscope-ai/TQ-Claw/projects) or roadmap labels.
...
TQ-Claw is designed to be **extensible**: you can add models, channels, Skills, and more. Below are the main contribution areas we care about.

---

### Adding New Models / Model Providers

TQ-Claw supports **multiple model backends**: cloud APIs (e.g. DashScope, ModelScope), **Ollama**, and local backends (**llama.cpp**, **MLX**). You can contribute in two ways:
...
1. **Provider definition** (in `src/tqclaw/providers/registry.py` or equivalent):
...
2. **Chat model class** (if the API is not OpenAI-compatible):
   - Implement a class inheriting from `agentscope.model.ChatModelBase` (or TQ-Claw’s local/remote wrappers where applicable).
   - Support streaming and non-streaming if the agent uses both; respect `tool_choice` and tools API if used.
   - Register the class in the registry’s chat model map so the runtime can resolve it by name (see `_CHAT_MODEL_MAP` in `src/tqclaw/providers/registry.py`).
...
Channels are how TQ-Claw talks to **DingTalk, Feishu, QQ, Discord, iMessage**, etc. You can add a new channel so TQ-Claw can work with your favorite IM or bot platform.
...
- **Implementation:** Implement a **subclass of `BaseChannel`** (in `src/tqclaw/app/channels/base.py`):
...
- **Discovery:** Built-in channels are registered in `src/tqclaw/app/channels/registry.py`. **Custom channels** are loaded from the working directory: place a module (e.g. `custom_channels/telegram.py` or a package `custom_channels/telegram/`) that defines a `BaseChannel` subclass with a `channel` attribute.
- **CLI:** Users install/add channels with:
  - `tqclaw channels install <key>` — create a template or copy from `--path` / `--url`
  - `tqclaw channels add <key>` — install and add to config
  - `tqclaw channels remove <key>` — remove custom channel from `custom_channels/`
  - `tqclaw channels config` — interactive config
...
**Skills** define what TQ-Claw can do: cron, file reading, PDF/Office, news, browser, etc. We welcome **broadly useful** base skills (productivity, documents, communication, automation) that fit the majority of users.
...
- **Location:** Built-in skills live under `src/tqclaw/agents/skills/<skill_name>/`. The app merges built-in and user **customized_skills** from the working dir into **active_skills**; no extra registration is needed beyond placing a valid `SKILL.md` in a directory.
...
- **Skills Hub:** TQ-Claw supports importing skills from a community hub (e.g. ClawHub). If you want your skill to be installable via hub, follow the same `SKILL.md` + `references/`/`scripts/` layout and the hub’s packaging format.
...
TQ-Claw aims to run on **Windows**, **Linux**, and **macOS**. Contributions that improve support on a specific platform are welcome.
...
- **Install and run:** One-line install (`install.sh`), `pip` install, and `tqclaw init` / `tqclaw app` should work (or be clearly documented) on each supported platform. Fixes to install or startup on a given OS are valuable.
...
- **MCP (Model Context Protocol):** TQ-Claw supports runtime **MCP tool** discovery and hot-plug. Contributing new MCP servers or tools (or docs on how to attach them) helps users extend the agent without changing core code.
- **Documentation:** Fixes and improvements to [the docs](https://tqclaw.agentscope.io/) (under `website/public/docs/`) and README are always welcome.
...
- **Discussions:** [GitHub Discussions](https://github.com/agentscope-ai/TQ-Claw/discussions)
- **Bugs and features:** [GitHub Issues](https://github.com/agentscope-ai/TQ-Claw/issues)
- **Community:** DingTalk group (see [README](README.md)) and [Discord](https://discord.gg/eYMpfnkG8h)

Thank you for contributing to TQ-Claw. Your work helps make it a better assistant for everyone. 🐾

...
[[ドキュメント](https://tqclaw.agentscope.io/)] [[English README](README.md)] [[中文 README](README_zh.md)]
...
[2026-03-12] v0.0.7をリリースしました！詳細は[v0.0.7リリースノート](https://agentscope-ai.github.io/TQ-Claw/release-notes)でご確認ください。
...
[2026-03-09] v0.0.6をリリースしました！詳細は[v0.0.6リリースノート](https://agentscope-ai.github.io/TQ-Claw/release-notes)でご確認ください。

[2026-03-06] v0.0.5をリリースしました！詳細は[v0.0.5リリースノート](https://agentscope-ai.github.io/TQ-Claw/release-notes)でご確認ください。

[2026-03-02] v0.0.4をリリースしました！詳細は[v0.0.4リリースノート](https://agentscope-ai.github.io/TQ-Claw/release-notes)でご確認ください。
...
> - **DingTalk / Feishu / QQ でチャットしたい**: コンソールで[チャネル](https://tqclaw.agentscope.io/docs/channels)を設定。
> - **Pythonをインストールしたくない**: [ワンラインインストール](#ワンラインインストールベータ版継続的に改善中)がPythonを自動処理。または[ModelScopeワンクリック](https://modelscope.cn/studios/fork?target=AgentScope/TQ-Claw)でクラウドデプロイ。
...
ブラウザで **http://127.0.0.1:8088/** を開くとコンソール（TQ-Clawとのチャット、エージェントの設定）が利用できます。DingTalk、Feishu、QQなどで会話するには、[チャネルドキュメント](https://tqclaw.agentscope.io/docs/channels)でチャネルを接続してください。
...
>    - **インストールディレクトリを確認**：
>      - `uv` の利用可否を確認：CMD で `uv --version` と入力し、バージョン番号が表示された場合は**TQ-Claw パスのみ設定**。`『uv』 は内部コマンドでも外部コマンドでもなく、実行可能プログラムまたはバッチファイルでもありません。` と表示された場合は両方を設定する必要があります。
...
>      - TQ-Clawパス：通常 `%USERPROFILE%\.tqclaw\bin` にあります。
>    - **システム環境変数Pathへの手動追加**：
...
>   - **`uv`環境変数の設定**：`uv`の配置ディレクトリと `%USERPROFILE%\.tqclaw\bin` をシステムの `Path` 変数に追加してください。
>   - **再実行**：新しいターミナルを開き、インストールスクリプトを再度実行して `TQ-Claw` のインストールを完了させてください。
>   - **`TQ-Claw`環境変数の設定**：`%USERPROFILE%\.tqclaw\bin` をシステムの `Path` 変数に追加します。

インストール完了後、新しいターミナルを開き、以下を実行してください：

```bash
tqclaw init --defaults   # または: tqclaw init（対話式）
tqclaw app
```
...
# アンインストール
tqclaw uninstall          # 設定とデータを保持
tqclaw uninstall --purge  # すべて削除
```
...
コマンドラインツールに慣れていない場合、TQ-Clawのデスクトップアプリケーションをダウンロードして使用できます。Python環境の手動設定やコマンドの実行は不要です。

#### ダウンロード

[GitHub Releases](https://github.com/agentscope-ai/TQ-Claw/releases)からデスクトップアプリをダウンロード:
- **Windows**: `TQ-Claw-Setup-<version>.exe`
- **macOS**: `TQ-Claw-<version>-macOS.zip` (Apple Silicon推奨)
...
ReleasesからTQ-Claw macOSアプリをダウンロードすると、macOSは次のように表示する場合があります: *「Appleは'TQ-Claw'に悪意のあるソフトウェアが含まれていないことを確認できません」*。これはアプリが公証されていないためです。以下の方法で開くことができます:

- **右クリックして開く（推奨）**
  TQ-Clawアプリを右クリック（またはControl+クリック）→ **「開く」** → ダイアログで再度 **「開く」** をクリック。これによりGatekeeperにアプリを信頼していることを伝えます。その後は通常通りダブルクリックで起動できます。

- **システム設定で許可**
  それでもブロックされる場合、**システム設定 → プライバシーとセキュリティ** に移動し、*「'TQ-Claw'は未確認の開発元からのものであるためブロックされました」* のようなメッセージまでスクロールし、**「このまま開く」** または **「許可」** をクリックします。
...
詳細な使用方法、トラブルシューティング、よくある問題については、[デスクトップアプリケーションガイド](https://tqclaw.agentscope.io/docs/desktop)を参照してください。

### Dockerを使用

イメージは **Docker Hub**（`agentscope/tqclaw`）で公開しています。タグ: `latest`（安定版）; `pre`（PyPIプレリリース版）。
...
中国のユーザーは阿里雲コンテナレジストリ（ACR）も利用できます: `agentscope-registry.ap-southeast-1.cr.aliyuncs.com/agentscope/tqclaw`（タグは同じ）。

ブラウザで **http://127.0.0.1:8088/** を開くとコンソールが利用できます。設定、メモリ、スキルは `tqclaw-data` ボリュームに保存されます。モデル設定とAPIキーは `tqclaw-secrets` ボリュームに保存されます。APIキー（例: `DASHSCOPE_API_KEY`）を渡すには、`docker run` に `-e VAR=value` または `--env-file .env` を追加してください。

> **ホストマシン上のOllamaや他のモデルサービスに接続する**
>
> Dockerコンテナ内の `localhost` はコンテナ自身を指し、ホストマシンではありません。Ollama（または他のモデルサービス）がホスト上で動作している場合、以下のいずれかの方法でTQ-Clawコンテナからアクセスできます：
>
...
>   agentscope/tqclaw:latest
> ```
> その後、TQ-Clawの **Settings → Models → Ollama** で、Base URLを `http://host.docker.internal:11434` または対応するポートに変更してください。
>
...
**ローカルインストール不要？** [ModelScope Studio](https://modelscope.cn/studios/fork?target=AgentScope/TQ-Claw) でワンクリッククラウドセットアップ。他の人があなたのTQ-Clawを操作できないよう、Studioを**非公開**に設定してください。

### Alibaba Cloud ECSへのデプロイ

TQ-ClawをAlibaba Cloud（ECS）で実行するには、ワンクリックデプロイを使用します: [TQ-Claw on Alibaba Cloud (ECS) デプロイリンク](https://computenest.console.aliyun.com/service/instance/create/cn-hangzhou?type=user&ServiceId=service-1ed84201799f40879884)を開き、プロンプトに従ってください。ステップバイステップの手順については、[Alibaba Cloud Developer: 3分でAIアシスタントをデプロイ](https://developer.aliyun.com/article/1713682)を参照してください。

---

## APIキー

**クラウドLLM**（例: DashScope、ModelScope）を使用する場合、チャットの前にAPIキーを設定する必要があります。有効なキーが設定されるまでTQ-Clawは動作しません。詳細は[公式ドキュメント](https://tqclaw.agentscope.io/docs/models#configure-cloud-providers)をご覧ください。

**設定方法:**

1. **コンソール（推奨）** — `tqclaw app` 実行後、**http://127.0.0.1:8088/** を開き → **設定** → **モデル**。プロバイダーを選択し、**APIキー**を入力して、そのプロバイダーとモデルを有効にしてください。
2. **`tqclaw init`** — `tqclaw init` を実行すると、LLMプロバイダーとAPIキーの設定が案内されます。プロンプトに従ってプロバイダーを選択し、キーを入力してください。
3. **環境変数** — DashScopeの場合、シェルまたはワーキングディレクトリの `.env` ファイルで `DASHSCOPE_API_KEY` を設定できます。

その他のキー（例: Web検索用 `TAVILY_API_KEY`）は、コンソールの **設定 → 環境変数** で設定するか、[Config](https://tqclaw.agentscope.io/docs/config) で詳細を確認してください。

> **ローカルモデルのみ使用する場合:** [ローカルモデル](#ローカルモデル)（llama.cppまたはMLX）を使用する場合、APIキーは**不要**です。

## ローカルモデル

TQ-ClawはLLMを完全にローカルマシン上で実行できます — APIキーやクラウドサービスは不要です。詳細は[公式ドキュメント](https://tqclaw.agentscope.io/docs/models#local-providers-llamacpp--mlx)をご覧ください。

| バックエンド       | 最適な用途                                 | インストール                                                              |
| ------------- | ---------------------------------------- | -------------------------------------------------------------------- |
| **llama.cpp** | クロスプラットフォーム（macOS / Linux / Windows） | `pip install 'tqclaw[llamacpp]'` または `bash install.sh --extras llamacpp` |
| **MLX**       | Apple Silicon（M1/M2/M3/M4）             | `pip install 'tqclaw[mlx]'` または `bash install.sh --extras mlx`         |
| **Ollama**    | クロスプラットフォーム（Ollamaサービスが必要） | `pip install 'tqclaw[ollama]'` または `bash install.sh --extras ollama`   |

インストール後、**コンソール**UIでローカルモデルのダウンロードと管理ができます。コマンドラインでも利用できます:

```bash
tqclaw models download Qwen/Qwen3-4B-GGUF
tqclaw models # ダウンロードしたモデルを選択
tqclaw app    # サーバーを起動
```
...
| トピック                                                                 | 説明                                                |
| ------------------------------------------------------------------------- | --------------------------------------------------- |
| [はじめに](https://tqclaw.agentscope.io/docs/intro)                        | TQ-Clawとは何か、使い方                               |
| [クイックスタート](https://tqclaw.agentscope.io/docs/quickstart)          | インストールと実行（ローカルまたはModelScope Studio） |
| [コンソール](https://tqclaw.agentscope.io/docs/console)                     | Web UI: チャットとエージェント設定                  |
| [モデル](https://tqclaw.agentscope.io/docs/models)                         | クラウド・ローカル・カスタムプロバイダーの設定       |
| [チャネル](https://tqclaw.agentscope.io/docs/channels)                      | DingTalk、Feishu、QQ、Discord、iMessageなど         |
| [スキル](https://tqclaw.agentscope.io/docs/skills)                          | 機能の拡張とカスタマイズ                             |
| [MCP](https://tqclaw.agentscope.io/docs/mcp)                                | MCPクライアントの管理                               |
| [メモリ](https://tqclaw.agentscope.io/docs/memory)                          | 長期記憶                              |
| [コンテキスト](https://tqclaw.agentscope.io/docs/context)                  | コンテキスト管理メカニズム                          |
| [魔法コマンド](https://tqclaw.agentscope.io/docs/commands)                 | AIの応答を待たずに会話状態を制御                     |
| [ハートビート](https://tqclaw.agentscope.io/docs/heartbeat)                 | スケジュールされたチェックインとダイジェスト        |
| [設定とワーキングディレクトリ](https://tqclaw.agentscope.io/docs/config)   | ワーキングディレクトリと設定ファイル                 |
| [CLI](https://tqclaw.agentscope.io/docs/cli)                                | Init、cronジョブ、スキル、クリーン                   |
| [FAQ](https://tqclaw.agentscope.io/docs/faq)                               | よくある質問とトラブルシューティング                 |
...
よくある質問、トラブルシューティングのヒント、既知の問題については、**[FAQページ](https://tqclaw.agentscope.io/docs/faq)** をご覧ください。
...
| **大小モデル協調** | TQ-Claw ワークフローと機密データ向けのローカル小モデル学習・ファインチューニング | 進行中 |
...
TQ-Clawはオープンに開発しており、あらゆる形の貢献を歓迎しています！上記の[ロードマップ](#ロードマップ)（特に**貢献者募集中**の項目）から興味のある領域を選び、[CONTRIBUTING](https://github.com/agentscope-ai/TQ-Claw/blob/main/CONTRIBUTING.md)を読んで始めてください。特に歓迎するのは：

- **横展開** — 新規チャネル、モデルプロバイダー、スキル、MCP。
- **既存機能の拡張・改善** — 表示とインタラクションの最適化、ダウンロードヒント、Windowsパス互換など。

[GitHub Discussions](https://github.com/agentscope-ai/TQ-Claw/discussions)で議論に参加し、アイデアを提案したりタスクを担当したりしてください。
...
- **その後**: `tqclaw init --defaults` を実行し、次に `tqclaw app` を実行。

---

## なぜTQ-Claw？

TQ-Clawは **Co Personal Agent Workstation**（共同パーソナルエージェントワークステーション）であると同時に、「co-paw」— いつもあなたのそばにいるパートナーを表しています。単なる冷たいツールではなく、TQ-Clawはいつでも手（または肉球！）を貸してくれる温かい「小さな肉球」です。デジタルライフにおける究極のチームメイトです。
...
TQ-Clawは `tqclaw init` 実行時に**匿名**の利用データを収集し、ユーザー環境の把握と製品改善に役立てています。データは**バージョンごとに1回**送信されます — TQ-Clawをアップグレードすると、バージョン分布を把握するために再収集されます。

**収集する情報：**

- TQ-Clawバージョン（例: 0.0.7）
...
`tqclaw init` を対話モードで実行すると、同意するかどうか尋ねられます。`--defaults` モードでは自動的に同意されます。プロンプトはバージョンごとに1回のみ表示され、TQ-Clawの機能には影響しません。

---

## ライセンス

TQ-Clawは[Apache License 2.0](LICENSE)の下でリリースされています。

---

## コントリビューター

TQ-Clawをより良くするために貢献してくださったすべての方々に感謝します：

<a href="https://github.com/agentscope-ai/TQ-Claw/graphs/contributors">
  <img src="https://contrib.rocks/image?repo=agentscope-ai/TQ-Claw" alt="コントリビューター" />
</a>

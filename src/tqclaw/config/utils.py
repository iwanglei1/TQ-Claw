...
def _normalize_working_dir_bound_paths(data: object) -> object:
    """Normalize legacy ~/.tqclaw-bound paths to current WORKING_DIR.

    This keeps TQCLAW_WORKING_DIR effective even if user config files contain
    older hard-coded paths like "~/.tqclaw/media" or
    "/Users/x/.tqclaw/workspaces/...".
    Only rewrites known working-dir-bound keys.
    """
    legacy_root_tilde = "~/.tqclaw"
...
def get_available_channels() -> Tuple[str, ...]:
    ...
    tqclaw.channels), filtered by TQCLAW_ENABLED_CHANNELS or
    TQCLAW_DISABLED_CHANNELS when set.

    * TQCLAW_ENABLED_CHANNELS — whitelist (only these channels are active).
    * TQCLAW_DISABLED_CHANNELS — blacklist (all channels *except* these).
    * If both are set, TQCLAW_ENABLED_CHANNELS takes precedence.
...
def is_running_in_container() -> bool:
    ...
    Prefer env TQCLAW_RUNNING_IN_CONTAINER (1/true/yes) at call time so

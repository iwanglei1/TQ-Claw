...
class VoiceChannelConfig(BaseChannelConfig):
    ...
    language: str = "en-US"
    welcome_greeting: str = "Hi! This is TQ-Claw. How can I help you?"
...
def load_agent_config(agent_id: str) -> AgentProfileConfig:
    ...
    # This keeps TQCLAW_WORKING_DIR effective even if existing agent.json
    # contains older hard-coded paths like "~/.tqclaw/media".
    try:
...
def migrate_legacy_config_to_multi_agent() -> bool:
    ...
    default_agent_config = AgentProfileConfig(
        id="default",
        name="Default Agent",
        description="Default TQ-Claw agent",
    ...
    # When TQCLAW_WORKING_DIR is customized, historical data may still exist
    # under "~/.tqclaw".
    old_workspace = Path("~/.tqclaw").expanduser().resolve()

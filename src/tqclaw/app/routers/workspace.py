...
def _extract_and_merge_zip(data: bytes, workspace_dir: Path) -> None:
    ...
    tmp_dir = None
    try:
        tmp_dir = Path(tempfile.mkdtemp(prefix="tqclaw_upload_"))
        with zipfile.ZipFile(io.BytesIO(data)) as zf:
...
async def download_workspace(request: Request):
    ...
    filename = f"tqclaw_workspace_{agent.agent_id}_{timestamp}.zip"
...
@router.post(
    ...
    description=(
        ...
        "the zip are left unchanged (e.g. tqclaw.db, runtime dirs). "

...
Shared by in-chat /daemon <sub> and CLI `tqclaw daemon <sub>`.
Logs: tail WORKING_DIR / "tqclaw.log". Restart: in-process reload of channels,
...
async def run_daemon_restart(context: DaemonContext) -> str:
    ...
    return (
        ...
        "Run the app (e.g. `tqclaw app`) and use /daemon restart in chat, "
...
def run_daemon_version(context: DaemonContext) -> str:
    ...
    return (
        ...
        f"- Log file: {context.working_dir / 'tqclaw.log'}"
    )


def run_daemon_logs(context: DaemonContext, lines: int = 100) -> str:
    """Tail last N lines from WORKING_DIR / tqclaw.log."""
    log_path = context.working_dir / "tqclaw.log"

...
from ..utils.logging import setup_logger, add_tqclaw_file_handler
...
async def lifespan(
    ...
    add_tqclaw_file_handler(WORKING_DIR / "tqclaw.log")
...
# Console static dir: env, or tqclaw package data (console), or cwd.
_CONSOLE_STATIC_ENV = "TQCLAW_CONSOLE_STATIC_DIR"


def _resolve_console_static_dir() -> str:
    if os.environ.get(_CONSOLE_STATIC_ENV):
        return os.environ[_CONSOLE_STATIC_ENV]
    # Shipped dist lives in tqclaw package as static data (not a Python pkg).
    pkg_dir = Path(__file__).resolve().parent.parent
    candidate = pkg_dir / "console"
    if candidate.is_dir() and (candidate / "index.html").exists():
        return str(candidate)
    # the following code can be removed after next release,
    # because the console will be output to tqclaw's
    # `src/tqclaw/console/` directory directly by vite.
    cwd = Path(os.getcwd())
...
def read_root():
    ...
    return {
        "message": (
            "TQ-Claw Web Console is not available. "
            "If you installed TQ-Claw from source code, please run "
            "`npm ci && npm run build` in TQ-Claw's `console/` "
            "directory, and restart TQ-Claw to enable the "
            "web console."
        ),
    }
...
@app.get("/api/version")
def get_version():
    """Return the current TQ-Claw version."""
...
if os.path.isdir(_CONSOLE_STATIC_DIR):
    ...
    @app.get("/tqclaw-symbol.svg")
    def _console_icon():
        f = _console_path / "tqclaw-symbol.svg"
        if f.is_file():
            return FileResponse(f, media_type="image/svg+xml")
        raise HTTPException(status_code=404, detail="Not Found")

    @app.get("/tqclaw-dark.png")
    def _console_dark_icon():
        f = _console_path / "tqclaw-dark.png"
        if f.is_file():
            return FileResponse(f, media_type="image/png")
        raise HTTPException(status_code=404, detail="Not Found")

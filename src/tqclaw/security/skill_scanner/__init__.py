# -*- coding: utf-8 -*-
"""
Skill security scanner for TQ-Claw.
...
def _get_scan_mode(cfg: Any = None) -> str:
    ...
    Priority: env ``TQCLAW_SKILL_SCAN_MODE`` > config > default ``warn``.
    """
    env = os.environ.get("TQCLAW_SKILL_SCAN_MODE")
    if env is not None:
        val = env.lower().strip()
...
def _get_blocked_history_path() -> Path:
    try:
        ...
        return Path.home() / ".tqclaw" / _BLOCKED_HISTORY_FILE

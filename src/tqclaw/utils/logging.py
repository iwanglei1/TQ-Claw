# -*- coding: utf-8 -*-
"""Logging setup for TQ-Claw: console output and optional file handler."""
import io
import logging
...
_TQCLAW_LOG_MAX_BYTES = 5 * 1024 * 1024  # 5 MiB
_TQCLAW_LOG_BACKUP_COUNT = 3
...
def setup_logger(level: int | str = logging.INFO):
    """Configure logging to only output from this package (tqclaw), not deps."""
    ...
    # Only attach handler to our namespace so only tqclaw.* logs are printed.
...
def add_tqclaw_file_handler(log_path: Path) -> None:
    """Add a file handler to the tqclaw logger for daemon logs.
    ...
    Args:
        log_path: Path to the log file (e.g. WORKING_DIR / "tqclaw.log").
    """
    ...
    if is_windows_or_linux:
        ...
        file_handler = logging.handlers.RotatingFileHandler(
            log_path,
            encoding="utf-8",
            maxBytes=_TQCLAW_LOG_MAX_BYTES,
            backupCount=_TQCLAW_LOG_BACKUP_COUNT,
        )

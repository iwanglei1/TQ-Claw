...
TELEMETRY_ENDPOINT = "https://tqclaw-telemetry-xissagieap.cn-hangzhou.fcapp.run"
...
def _detect_install_method() -> str:
    """Detect how TQ-Claw was installed based on environment signals."""
    import os

    if os.environ.get("TQCLAW_RUNNING_IN_CONTAINER", "").lower() in (
        "1",
        "true",
        "yes",
    ):
        return "docker"
    if os.environ.get("TQCLAW_DESKTOP_APP", "").lower() in (
        "1",
        "true",
        "yes",
    ):
...
def get_system_info() -> dict[str, Any]:
    ...
    - tqclaw_version: TQ-Claw version string
    - install_method: How TQ-Claw was installed (docker/desktop/pip)
    - os: Operating system (Windows/Darwin/Linux)
    - os_version: OS version string
    - python_version: Python version running tqclaw (major.minor)
    - architecture: CPU architecture (x86_64/arm64/etc)
    - has_gpu: GPU availability detection
    """
    from ..__version__ import __version__ as tqclaw_ver

    info = {
        "install_id": str(uuid.uuid4()),
        "tqclaw_version": _safe_get(lambda: tqclaw_ver, "unknown"),
...
def _get_current_version() -> str:
    """Get the current TQ-Claw version string."""
    try:
        from ..__version__ import __version__ as tqclaw_ver
...
def has_telemetry_been_collected(working_dir: Path) -> bool:
    ...
    Re-triggers collection when TQ-Claw is upgraded (or downgraded) to a version
    that hasn't been collected before.

    Args:
        working_dir: Path to TQ-Claw working directory
    ...
    try:
        ...
        # v1.1 compat: single tqclaw_version field
        return marker_data.get("tqclaw_version", "") == current
    except Exception:
        return False
        ...
        if marker_file.exists():
            try:
                ...
                if not collected_versions:
                    old_ver = old_data.get("tqclaw_version", "")
                    if old_ver:

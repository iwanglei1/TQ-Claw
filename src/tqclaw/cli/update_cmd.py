...
from .process_utils import (
    ...
    _is_tqclaw_service_command,
    _process_table,
)

_PYPI_JSON_URL = "https://pypi.org/pypi/tqclaw/json"
...
class InstallInfo:
    """Information about the current TQ-Claw installation."""
...
class RunningServiceInfo:
    """Detected TQ-Claw service endpoint state."""
...
def _fetch_latest_version() -> str:
    """Fetch the latest published TQ-Claw version from PyPI."""
    try:
        resp = httpx.get(
            _PYPI_JSON_URL,
            timeout=10.0,
        ...
        raise click.ClickException(
            f"Failed to fetch the latest TQ-Claw version from PyPI: {exc}",
        ) from exc
    except json.JSONDecodeError as exc:
        raise click.ClickException(
            "Received an invalid response from PyPI when checking for the "
            f"latest TQ-Claw version: {exc}",
        ) from exc
    version = str(data.get("info", {}).get("version", "")).strip()
    if not version:
        raise click.ClickException(
            "Unable to determine the latest TQ-Claw version.",
        )
    return version
...
def _detect_installation() -> InstallInfo:
    ...
    dist = metadata.distribution("tqclaw")
...
def _probe_service(base_url: str) -> RunningServiceInfo:
    """Probe a possible running TQ-Claw HTTP service."""
    try:
        resp = httpx.get(
...
def _process_candidate_ports() -> list[int]:
    """Infer candidate local TQ-Claw service ports from running processes."""
    ports: list[int] = []
    for _pid, command in _process_table():
        if not _is_tqclaw_service_command(command):
            continue
...
def _detect_running_service(
    ...
    """Detect whether a TQ-Claw HTTP service is currently running."""
    candidates: list[str] = []
    seen: set[str] = set()
    preferred_hosts: list[str] = []
...
def _running_service_display(running: RunningServiceInfo) -> str:
    ...
    if not running.base_url:
        return "a running TQ-Claw service"
    version_suffix = f" (version {running.version})" if running.version else ""
    return f"TQ-Claw service at {running.base_url}{version_suffix}"


def _confirm_force_shutdown(running: RunningServiceInfo) -> bool:
    """Ask whether `tqclaw shutdown` should be used before updating."""
    click.echo("")
    click.secho("!" * 72, fg="yellow", bold=True)
    click.secho(
        "WARNING: RUNNING TQCLAW SERVICE DETECTED",
        fg="yellow",
        bold=True,
    )
    click.secho("!" * 72, fg="yellow", bold=True)
    ...
    click.secho(
        "Running `tqclaw shutdown` will forcibly terminate the current "
        "TQ-Claw backend/frontend processes.",
        fg="red",
        bold=True,
    )
    click.secho(
    ...
    return click.confirm(
        "Run `tqclaw shutdown` now and continue with the update?",
        default=False,
    )
...
def _run_shutdown_for_update(
    ...
    """Run `tqclaw shutdown` in the current environment before updating."""
    command = [info.python_executable, "-m", "tqclaw"]
    ...
    click.echo("")
    click.echo("Running `tqclaw shutdown` before updating...")
    ...
    try:
        ...
        raise click.ClickException(
            "Failed to run `tqclaw shutdown`: " f"{exc}",
        ) from exc
    ...
    if result.returncode != 0:
        raise click.ClickException(
            "`tqclaw shutdown` failed. Please stop the running TQ-Claw "
            "service manually before running `tqclaw update`.",
        )
...
def _build_upgrade_command(
    ...
    package_spec = f"tqclaw=={latest_version}"
...
def _spawn_update_worker(
    ...
    *,
    ...
    worker_code = (
        "from tqclaw.cli.update_cmd import run_update_worker; "
        "import sys; "
...
def _run_update_worker_foreground(plan_path: Path) -> int:
    ...
    try:
        ...
        click.echo("")
        click.echo("[tqclaw] Update interrupted. Stopping installer...")
...
def run_update_worker(plan_path: str | Path) -> int:
    ...
    click.echo("")
    click.echo(
        "[tqclaw] Updating TQ-Claw "
        f"{plan['current_version']} -> {plan['latest_version']}...",
    )
    click.echo(f"[tqclaw] Using installer: {plan['installer_label']}")
    ...
    try:
        ...
        click.echo(f"[tqclaw] Update failed: {exc}")
        return_code = 1
    finally:
        try:
    ...
    if return_code == 0:
        click.echo("[tqclaw] Update completed successfully.")
        click.echo(
            "[tqclaw] Please restart any running TQ-Claw service "
            "to use the new version.",
        )
    else:
        click.echo(f"[tqclaw] Update failed with exit code {return_code}.")
        click.echo(
            "[tqclaw] Please fix the error above and run "
            "`tqclaw update` again.",
        )
...
def update_cmd(ctx: click.Context, yes: bool) -> None:
    """Upgrade TQ-Claw in the current Python environment."""
    ...
    if version_check is False:
        click.echo("TQ-Claw is already up to date.")
        return
    ...
    if running.is_running:
        if yes:
            raise click.ClickException(
                ...
                "Please stop it before running `tqclaw update`, or rerun "
                "without `--yes` to confirm a forced `tqclaw shutdown`.",
            )
        ...
        if running.is_running:
            raise click.ClickException(
                "Detected "
                f"{_running_service_display(running)} after `tqclaw shutdown`. "
                "Please stop it manually before running `tqclaw update`.",
            )

    if not yes and not click.confirm(
        f"Update TQ-Claw to {latest_version} in the current environment?",
        default=True,
    ):
        click.echo("Cancelled.")
        return

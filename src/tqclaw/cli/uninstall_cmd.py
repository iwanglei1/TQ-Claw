# -*- coding: utf-8 -*-
"""tqclaw uninstall — remove the TQ-Claw environment and CLI wrapper."""
...
def _remove_path_entry(profile: Path) -> bool:
    """
    Remove TQ-Claw PATH lines from a shell profile. Returns True if changed.
    """
    if not profile.is_file():
        return False

    text = profile.read_text()
    # Remove the "# TQ-Claw" comment line and the export PATH line
    cleaned = re.sub(
        r"\n?# TQ-Claw\nexport PATH=\"\$HOME/\.tqclaw/bin:\$PATH\"\n?",
        "\n",
        text,
    )
    if cleaned == text:
...
def uninstall_cmd(purge: bool, yes: bool) -> None:
    """Remove TQ-Claw environment, CLI wrapper, and shell PATH entries."""
    wd = WORKING_DIR

    if purge:
        click.echo(f"This will remove ALL TQ-Claw data in {wd}")
    else:
        click.echo(
            "This will remove the TQ-Claw Python environment and CLI wrapper.",
        )
    ...
    click.echo("")
    click.echo("TQ-Claw uninstalled. Please restart your terminal.")

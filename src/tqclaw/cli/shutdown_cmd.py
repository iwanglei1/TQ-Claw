...
def _find_frontend_dev_pids() -> set[int]:
    ...
    for pid, command in _process_table():
        ...
        if "vite" in lowered and console_dir in lowered:
            matches.add(pid)
            continue
        if "tqclaw-console" in lowered and (
            "npm" in lowered
            or "pnpm" in lowered
            or "yarn" in lowered
            or "node" in lowered
...
def _find_desktop_wrapper_pids() -> set[int]:
    """Find `tqclaw desktop` wrapper processes for this project."""
    matches: set[int] = set()
    patterns = (
        " -m tqclaw desktop",
        " tqclaw desktop",
...
def _find_windows_wrapper_ancestor_pids(pids: set[int]) -> set[int]:
    """Find TQ-Claw wrapper/supervisor ancestors for Windows backend PIDs."""
    ...
    for pid in pids:
        ...
        while True:
            ...
            if _is_tqclaw_wrapper_process(parent_info[1], parent_info[2]):
...
@click.command("shutdown", help="Force stop the running TQ-Claw app processes.")
@click.option(
    "--port",
    default=None,
    type=int,
...
def shutdown_cmd(ctx: click.Context, port: Optional[int]) -> None:
    """Stop the running TQ-Claw app processes.

    `tqclaw app` only starts the backend process. The web console is normally
    ...
    if not all_targets:
        raise click.ClickException(
            "No running TQ-Claw backend/frontend process was found.",
        )

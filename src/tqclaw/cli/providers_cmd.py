...
def configure_llm_slot_interactive(*, use_defaults: bool = False) -> None:
    ...
    if not eligible:
        if use_defaults:
            click.echo(
                "No LLM provider configured. Run 'tqclaw models config' "
                "to configure later.",
            )
            return
        click.echo(
    ...
    if not model and use_defaults:
        click.echo(
            ...
            "Run 'tqclaw models config' to set one.",
        )
        return
    try:
    ...
    for defn in _all_provider_objects(manager):
        ...
        if defn.is_local:
            ...
            if all_models:
                ...
                click.echo("  Use 'tqclaw models download' to add models.")
        else:
...
def add_provider_cmd(
    ...
    click.echo(
        "  Run 'tqclaw models add-model' to add models, "
        "then 'tqclaw models config-key' to set the API key.",
    )
...
def download_cmd(
    ...
    \b
    Examples:
      tqclaw models download TheBloke/Mistral-7B-Instruct-v0.2-GGUF
      tqclaw models download TheBloke/Mistral-7B-Instruct-v0.2-GGUF \\
    ...
    try:
        ...
        click.echo(
            click.style(
                ...
                "Install with: pip install 'tqclaw[local]'",
                fg="red",
            ),
        )
        raise SystemExit(1) from exc
    ...
    click.echo(
        ...
        f"  tqclaw models set-llm  (select '{backend}' provider)",
    )
...
def list_local_cmd(backend: str | None) -> None:
    ...
    if not models:
        ...
        click.echo("Use 'tqclaw models download <repo_id>' to download one.")
        return
...
def ollama_list_cmd() -> None:
    ...
    if not models:
        ...
        click.echo("Use 'tqclaw models ollama-pull <model>' to download one.")
        return
...
def ollama_remove_cmd(model_name: str, yes: bool) -> None:
    ...
    Examples:
      ...
      tqclaw models ollama-remove qwen2.5:3b -y
    """
    if not yes:

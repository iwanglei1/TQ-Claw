...
def _matches_tqclaw_cli_command(command: str, *subcommands: str) -> bool:
    """Return whether command line looks like a TQ-Claw CLI invocation."""
    lowered = f" {command.lower()}"
    return any(
        pattern in lowered
        for subcommand in subcommands
        for pattern in (
            f" -m tqclaw {subcommand}",
            f" tqclaw {subcommand}",
            f"__main__.py {subcommand}",
            f'tqclaw.exe" {subcommand}',
            f"tqclaw.exe {subcommand}",
        )
    )


def _is_tqclaw_service_command(command: str) -> bool:
    """Return whether the command line looks like a local TQ-Claw app."""
    return _matches_tqclaw_cli_command(command, "app")


def _is_tqclaw_wrapper_process(name: str, command: str) -> bool:
    """Return whether the process looks like a TQ-Claw CLI wrapper."""
    lowered_name = name.lower().removesuffix(".exe")
    return lowered_name == "tqclaw" or _matches_tqclaw_cli_command(
        command,
        "app",
        "desktop",
    )
...
def _candidate_hosts(host: str | None) -> list[str]:
    """Return host variants that can reach a local TQ-Claw service."""
    if not host:
        return []

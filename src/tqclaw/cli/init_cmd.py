...
TQ-Claw is a personal assistant that runs in your own environment. It can connect to
...
If multiple people can message the same TQ-Claw instance with tools enabled, they
share the same delegated authority (files, commands, secrets the agent can use).

If you are not comfortable with access control and hardening, do not run TQ-Claw with
...
TELEMETRY_INFO = """
Help improve TQ-Claw by sharing anonymous usage data!
...
This helps us understand TQ-Claw's usage environment and prioritize improvements.
"""
...
def _echo_telemetry_info_box() -> None:
    ...
    console.print(
        Panel(
            ...
            title="[bold]📊 Help improve TQ-Claw[/bold]",
            border_style="blue",
        ),
    )

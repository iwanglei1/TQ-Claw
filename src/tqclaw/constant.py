...
WORKING_DIR = (
    Path(EnvVarLoader.get_str("TQCLAW_WORKING_DIR", "~/.tqclaw"))
    .expanduser()
    .resolve()
)
SECRET_DIR = (
...
JOBS_FILE = EnvVarLoader.get_str("TQCLAW_JOBS_FILE", "jobs.json")

CHATS_FILE = EnvVarLoader.get_str("TQCLAW_CHATS_FILE", "chats.json")

TOKEN_USAGE_FILE = EnvVarLoader.get_str(
    "TQCLAW_TOKEN_USAGE_FILE",
    "token_usage.json",
)

CONFIG_FILE = EnvVarLoader.get_str("TQCLAW_CONFIG_FILE", "config.json")

HEARTBEAT_FILE = EnvVarLoader.get_str("TQCLAW_HEARTBEAT_FILE", "HEARTBEAT.md")
...
DOCS_ENABLED = EnvVarLoader.get_bool("TQCLAW_OPENAPI_DOCS", False)

# Memory directory
MEMORY_DIR = WORKING_DIR / "memory"

# Custom channel modules (installed via `tqclaw channels install`); manager
...
MEMORY_COMPACT_KEEP_RECENT = EnvVarLoader.get_int(
    "TQCLAW_MEMORY_COMPACT_KEEP_RECENT",
    3,
    min_value=0,
)
...
# Example: TQCLAW_CORS_ORIGINS="http://localhost:5173,http://127.0.0.1:5173"
# When unset, CORS middleware is not applied.
CORS_ORIGINS = EnvVarLoader.get_str("TQCLAW_CORS_ORIGINS", "").strip()

# LLM API retry configuration
LLM_MAX_RETRIES = EnvVarLoader.get_int(
    "TQCLAW_LLM_MAX_RETRIES",
    3,
    min_value=0,
)

LLM_BACKOFF_BASE = EnvVarLoader.get_float(
    "TQCLAW_LLM_BACKOFF_BASE",
    1.0,
    min_value=0.1,
)

LLM_BACKOFF_CAP = EnvVarLoader.get_float(
    "TQCLAW_LLM_BACKOFF_CAP",
    10.0,
    min_value=0.5,
)
...
try:
    TOOL_GUARD_APPROVAL_TIMEOUT_SECONDS = max(
        float(
            os.environ.get("TQCLAW_TOOL_GUARD_APPROVAL_TIMEOUT_SECONDS", "600"),
        ),
        1.0,
    )
except (TypeError, ValueError):

...
def _hub_http_timeout() -> float:
    raw = os.environ.get("TQCLAW_SKILLS_HUB_HTTP_TIMEOUT", "15")
    try:
...
def _hub_http_retries() -> int:
    raw = os.environ.get("TQCLAW_SKILLS_HUB_HTTP_RETRIES", "3")
    try:
...
def _hub_http_backoff_base() -> float:
    raw = os.environ.get("TQCLAW_SKILLS_HUB_HTTP_BACKOFF_BASE", "0.8")
    try:
...
def _hub_http_backoff_cap() -> float:
    raw = os.environ.get("TQCLAW_SKILLS_HUB_HTTP_BACKOFF_CAP", "6")
    try:
...
def _hub_base_url() -> str:
    return os.environ.get("TQCLAW_SKILLS_HUB_BASE_URL", "https://clawhub.ai")


def _hub_search_path() -> str:
    return os.environ.get(
        "TQCLAW_SKILLS_HUB_SEARCH_PATH",
        "/api/v1/search",
    )
...
def _hub_version_path() -> str:
    return os.environ.get(
        "TQCLAW_SKILLS_HUB_VERSION_PATH",
...
def _build_request(full_url: str, accept: str) -> Request:
    req = Request(
        ...
        headers={
            "Accept": accept,
            "User-Agent": "tqclaw-skills-hub/1.0",
        },
    )

# -*- coding: utf-8 -*-
"""Token counting utilities for TQ-Claw using HuggingFace tokenizers.
...
class TqclawTokenCounter(HuggingFaceTokenCounter):
    """Token counter for TQ-Claw with configurable tokenizer support.
...
_token_counter_cache: dict[tuple, TqclawTokenCounter] = {}


def _get_tqclaw_token_counter(
    ...
    Returns:
        TqclawTokenCounter: A token counter instance for the given
        configuration.
    ...
    if config_key not in _token_counter_cache:
        _token_counter_cache[config_key] = TqclawTokenCounter(

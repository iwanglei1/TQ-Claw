---
name: guidance
description: "回答用户关于 TQ-Claw 安装与配置的问题：优先定位并阅读本地文档，再提炼答案；若本地信息不足，兜底访问官网文档。"
metadata:
  {
...
当用户询问 **TQ-Claw 的安装、初始化、环境配置、依赖要求、常见配置项** 时，使用本 skill。
...
```bash
# 获取memory中的文档目录
DOC_DIR=$(find ~/.tqclaw/memory/ -type d -name "docs")
```
...
执行以下脚本逻辑来获取变量 $TQCLAW_ROOT：

```bash
# 获取二进制绝对路径
COP_PATH=$(which tqclaw 2>/dev/null || whereis tqclaw | awk '{print $2}')

# 逻辑推导：如果路径包含 .tqclaw/bin/tqclaw，则根目录在其上三层
# 例如：/path/to/TQ-Claw/.tqclaw/bin/tqclaw -> /path/to/TQ-Claw
if [[ "$COP_PATH" == *".tqclaw/bin/tqclaw" ]]; then
    TQCLAW_ROOT=$(echo "$COP_PATH" | sed 's/\/\.tqclaw\/bin\/tqclaw//')
else
    # 兜底：尝试获取所在目录的父目录
    TQCLAW_ROOT=$(dirname $(dirname "$COP_PATH") 2>/dev/null || echo ".")
fi

echo "Detected TQ-Claw Root: $TQCLAW_ROOT"
```

验证并列出文档目录：
使用推导出的 $TQCLAW_ROOT 定位文档：

```bash
# 组合标准文档路径
="$TQCLAW_ROOT/website/public/docs/"
...
if [ -d "$DOC_DIR" ]; then
    ...
    # 如果推导路径不对，执行全局模糊搜索
    find "$TQCLAW_ROOT" -type d -name "docs" | grep "website/public/docs"
fi
```
**如果项目文档不存在，搜索工作目录**

如果还是找不到文档，搜索 tqclaw 安装路径下的可用文档内容：

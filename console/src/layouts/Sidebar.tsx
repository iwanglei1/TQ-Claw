...
const PYPI_URL = "https://pypi.org/pypi/tqclaw/json";
...
docker run -p 127.0.0.1:8088:8088 -v tqclaw-data:/app/working agentscope/tqclaw:latest
\`\`\`

升级后重启服务 tqclaw app。`,

  ru: `### Как обновить TQ-Claw

Чтобы обновить TQ-Claw, выберите способ в зависимости от типа установки:
...
После обновления перезапустите сервис с помощью \`tqclaw app\`.`,

  en: `### How to update TQ-Claw
...
export default function Sidebar({ selectedKey }: SidebarProps) {
  ...
  const handleOpenUpdateModal = () => {
    ...
    const url = `https://tqclaw.agentscope.io/docs/faq.${faqLang}.md`;
    fetch(url, { cache: "no-cache" })
      .then((res) => (res.ok ? res.text() : Promise.reject()))
      .then((text) => {
        const zhPattern = /###\s*TQ-Claw如何更新[\s\S]*?(?=\n###|$)/;
        const enPattern = /###\s*How to update TQ-Claw[\s\S]*?(?=\n###|$)/;
        ...
        footer={[
          <Button
            ...
            onClick={() => {
              ...
              window.open(
                `https://tqclaw.agentscope.io/release-notes?lang=${websiteLang}`,
                "_blank",
              );
            }}

#!/usr/bin/env bash
# One-click build: console -> conda-pack -> TQ-Claw.app. Run from repo root.
...
# Launcher: force packed env; when no TTY log to ~/.tqclaw/desktop.log (no exec so we see errors)
cat > "${APP_DIR}/Contents/MacOS/${APP_NAME}" << 'LAUNCHER'
#!/usr/bin/env bash
ENV_DIR="$(cd "$(dirname "$0")/../Resources/env" && pwd)"
LOG="$HOME/.tqclaw/desktop.log"
unset PYTHONPATH
export PYTHONHOME="$ENV_DIR"
export TQCLAW_DESKTOP_APP=1
...
# Log level: env var TQCLAW_LOG_LEVEL or default to "info"
LOG_LEVEL="${TQCLAW_LOG_LEVEL:-info}"

if [ ! -t 2 ]; then
  mkdir -p "$HOME/.tqclaw"
  { echo "=== $(date) TQ-Claw starting ==="
  ...
  if [ ! -f "$HOME/.tqclaw/config.json" ]; then
    "$ENV_DIR/bin/python" -u -m tqclaw init --defaults --accept-security
  fi
  echo "Launching python with log-level=$LOG_LEVEL..."
  "$ENV_DIR/bin/python" -u -m tqclaw desktop --log-level "$LOG_LEVEL"
  EXIT=$?
  if [ $EXIT -ge 128 ]; then
    SIG=$((EXIT - 128))
...
exec "$ENV_DIR/bin/python" -u -m tqclaw desktop --log-level "$LOG_LEVEL"
LAUNCHER
...
if [[ -z "${VERSION}" ]]; then
  ...
  VERSION="$("${APP_DIR}/Contents/Resources/env/bin/python" -c \
    "from importlib.metadata import version; print(version('tqclaw'))" 2>/dev/null \
    || echo "0.0.0")"
...
<dict>
  ...
  <key>CFBundleIdentifier</key><string>com.tqclaw.desktop</string>
  <key>CFBundleName</key><string>${APP_NAME}</string>
  <key>CFBundleVersion</key><string>${VERSION}</string>
  <key>CFBundleShortVersionString</key><string>${VERSION}</string>
  ${ICON_PLIST}<key>NSHighResolutionCapable</key><true/>
  <key>LSMinimumSystemVersion</key><string>14.0</string>
  <key>NSDesktopFolderUsageDescription</key><string>TQ-Claw may access files in your Desktop folder if you use file-related features. You can choose Don'\''t Allow; the app will still run with limited file access.</string>
</dict>
</plist>
INFOPLIST
...
if [[ -n "${CREATE_ZIP}" ]]; then
  ZIP_NAME="${DIST}/TQ-Claw-${VERSION}-macOS.zip"

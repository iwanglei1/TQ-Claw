...
# -*- coding: utf-8 -*-
"""
Create a temporary conda env, install TQ-Claw from a wheel, run conda-pack.
...
def _pick_wheel(wheel_arg: str | None) -> Path:
    ...
    wheels = sorted(
        (REPO_ROOT / "dist").glob("tqclaw-*.whl"),
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="Conda-pack TQ-Claw (temp env).",
    )
    parser.add_argument(
        "--output",
        "-o",

...
$Archive = Join-Path $Dist "tqclaw-env.zip"
$Unpacked = Join-Path $Dist "win-unpacked"
$NsiPath = Join-Path $PackDir "tqclaw_desktop.nsi"
...
$VersionFile = Join-Path $RepoRoot "src\tqclaw\__version__.py"
$CurrentVersion = ""
...
$RunWheelBuild = $true
if ($CurrentVersion) {
  $wheelGlob = Join-Path $Dist "tqclaw-$CurrentVersion-*.whl"
  ...
  if ($existingWheels.Count -gt 0) {
    ...
    $oldWheels = Get-ChildItem -Path (Join-Path $Dist "tqclaw-*.whl") -ErrorAction SilentlyContinue
...
$LauncherBat = Join-Path $EnvRoot "TQ-Claw Desktop.bat"
@"
@echo off
cd /d "%~dp0"
...
REM Log level: env var TQCLAW_LOG_LEVEL or default to "info"
if not defined TQCLAW_LOG_LEVEL set "TQCLAW_LOG_LEVEL=info"

REM Set SSL certificate paths for packaged environment
REM Use temp file to avoid for /f blocking issue in bat scripts
set "CERT_TMP=%TEMP%\tqclaw_cert_%RANDOM%.txt"
...
if not exist "%USERPROFILE%\.tqclaw\config.json" (
  "%~dp0python.exe" -u -m tqclaw init --defaults --accept-security
)
"%~dp0python.exe" -u -m tqclaw desktop --log-level %TQCLAW_LOG_LEVEL%
"@ | Set-Content -Path $LauncherBat -Encoding ASCII

# Debug launcher .bat (shows console)
$DebugBat = Join-Path $EnvRoot "TQ-Claw Desktop (Debug).bat"
@"
@echo off
cd /d "%~dp0"
...
REM Debug mode: use debug log level by default (can override with TQCLAW_LOG_LEVEL)
if not defined TQCLAW_LOG_LEVEL set "TQCLAW_LOG_LEVEL=debug"
...
echo ====================================
echo TQ-Claw Desktop - Debug Mode
echo ====================================
echo Working Directory: %cd%
echo Python: "%~dp0python.exe"
echo PATH: %PATH%
echo Log Level: %TQCLAW_LOG_LEVEL%
...
echo [Launch] Starting TQ-Claw Desktop with log-level=%TQCLAW_LOG_LEVEL%...
echo Press Ctrl+C to stop
echo.
"%~dp0python.exe" -u -m tqclaw desktop --log-level %TQCLAW_LOG_LEVEL%
echo.
echo [Exit] TQ-Claw Desktop closed
pause
"@ | Set-Content -Path $DebugBat -Encoding ASCII

# VBScript launcher (no console window)
$LauncherVbs = Join-Path $EnvRoot "TQ-Claw Desktop.vbs"
@"
Set WshShell = CreateObject("WScript.Shell")
batPath = CreateObject("Scripting.FileSystemObject").GetParentFolderName(WScript.ScriptFullName) & "\TQ-Claw Desktop.bat"
...
if (-not $Version) {
  ...
  try {
    $Version = (& (Join-Path $EnvRoot "python.exe") -c "from importlib.metadata import version; print(version('tqclaw'))" 2>&1) -replace '\s+$', ''
...
Write-Host "[build_win] TQCLAW_VERSION=$Version OUTPUT_EXE will be under $Dist"
$OutInstaller = Join-Path (Join-Path $RepoRoot $Dist) "TQ-Claw-Setup-$Version.exe"

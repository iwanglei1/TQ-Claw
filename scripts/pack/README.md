# TQ-Claw Desktop packaging scripts
...
# Output: dist/TQ-Claw-Setup-<version>.exe
# Creates two launchers:
#   - TQ-Claw Desktop.vbs (silent, no console window)
#   - TQ-Claw Desktop (Debug).bat (shows console for troubleshooting)
...
APP_ENV="$(pwd)/dist/TQ-Claw.app/Contents/Resources/env"
...
When you **double-click** the .app and nothing appears, the launcher writes stderr/stdout to `~/.tqclaw/desktop.log`. Inspect that file for errors.
...
When users download the TQ-Claw macOS app (e.g. from Releases) as a `.app` (in a zip), macOS may show: *"Apple cannot verify that 'TQ-Claw' contains no malicious software"*. The app is not notarized. They can still open it as follows:

- **Right-click to open (recommended)**
  Right-click (or Control+click) the TQ-Claw app → **Open** → in the dialog click **Open** again. Gatekeeper will allow it; after that double-click works as usual.

- **Allow in System Settings**
  If still blocked, go to **System Settings → Privacy & Security**, find the message like *"TQ-Claw was blocked because it is from an unidentified developer"*, and click **Open Anyway** or **Allow**.

- **Remove quarantine attribute (not recommended for most users)**
  In Terminal: `xattr -cr /Applications/TQ-Claw.app` (or the path to the `.app` after unzipping). This clears the download quarantine flag; less safe than right-click → Open.
...
| File | Description |
|------|-------------|
| `build_common.py` | Create temporary conda env, install `tqclaw[full]` from a wheel, conda-pack; produces archive. |
| `build_macos.sh` | One-click: build wheel → build_common → unpack into TQ-Claw.app; optional zip. |
| `build_win.ps1` | One-click: build wheel → build_common → unpack → create VBS/BAT launchers → makensis installer. |
| `tqclaw_desktop.nsi` | NSIS script: pack `dist/win-unpacked`, add icons, and create shortcuts. |

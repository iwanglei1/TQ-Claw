...
info()  { printf "${GREEN}[tqclaw]${RESET} %s\n" "$*"; }
warn()  { printf "${YELLOW}[tqclaw]${RESET} %s\n" "$*"; }
error() { printf "${RED}[tqclaw]${RESET} %s\n" "$*" >&2; }
die()   { error "$@"; exit 1; }

# ── Defaults ──────────────────────────────────────────────────────────────────
TQCLAW_HOME="${TQCLAW_HOME:-$HOME/.tqclaw}"
TQCLAW_VENV="$TQCLAW_HOME/venv"
TQCLAW_BIN="$TQCLAW_HOME/bin"
PYTHON_VERSION="3.12"
TQCLAW_REPO="https://github.com/agentscope-ai/TQ-Claw.git"
...
printf "${GREEN}[tqclaw]${RESET} Installing TQ-Claw into ${BOLD}%s${RESET}\n" "$TQCLAW_HOME"
...
if [ -d "$TQCLAW_VENV" ]; then
    info "Existing environment found, upgrading..."
else
    info "Creating Python $PYTHON_VERSION environment..."
fi

uv venv "$TQCLAW_VENV" --python "$PYTHON_VERSION" --quiet

# Verify the venv was created
[ -x "$TQCLAW_VENV/bin/python" ] || die "Failed to create virtual environment"
info "Python environment ready ($("$TQCLAW_VENV/bin/python" --version))"
...
## Ensure console frontend assets are in src/tqclaw/console/ for source installs.
...
prepare_console() {
    ...
    local console_dest="$repo_dir/src/tqclaw/console"
...
cleanup_console() {
    ...
    if [ "$_CONSOLE_COPIED" = 1 ]; then
        rm -rf "$repo_dir/src/tqclaw/console/"*
    fi
}

if [ "$FROM_SOURCE" = true ]; then
    if [ -n "$SOURCE_DIR" ]; then
        info "Installing TQ-Claw from local source: $SOURCE_DIR"
        prepare_console "$SOURCE_DIR"
        info "Installing package from source..."
        uv pip install "${SOURCE_DIR}${EXTRAS_SUFFIX}" --python "$TQCLAW_VENV/bin/python" --prerelease=allow --index-url "$PYPI_MIRROR"
        cleanup_console "$SOURCE_DIR"
    else
        info "Installing TQ-Claw from source (GitHub)..."
        CLONE_DIR="$(mktemp -d)"
        trap 'rm -rf "$CLONE_DIR"' EXIT
        git clone --depth 1 "$TQCLAW_REPO" "$CLONE_DIR"
        prepare_console "$CLONE_DIR"
        info "Installing package from source..."
        uv pip install "${CLONE_DIR}${EXTRAS_SUFFIX}" --python "$TQCLAW_VENV/bin/python" --prerelease=allow --index-url "$PYPI_MIRROR"
        # CLONE_DIR is cleaned up by trap; no need for cleanup_console
    fi
else
    PACKAGE="tqclaw"
    if [ -n "$VERSION" ]; then
        PACKAGE="tqclaw==$VERSION"
    fi

    info "Installing ${PACKAGE}${EXTRAS_SUFFIX} from PyPI..."
    uv pip install "${PACKAGE}${EXTRAS_SUFFIX}" --python "$TQCLAW_VENV/bin/python" --prerelease=allow --quiet --index-url "$PYPI_MIRROR" --refresh-package tqclaw
fi

# Verify the CLI entry point exists
[ -x "$TQCLAW_VENV/bin/tqclaw" ] || die "Installation failed: tqclaw CLI not found in venv"
info "TQ-Claw installed successfully"
...
if [ "$_CONSOLE_AVAILABLE" = 0 ]; then
    ...
    CONSOLE_CHECK="$("$TQCLAW_VENV/bin/python" -c "import importlib.resources, tqclaw; p=importlib.resources.files('tqclaw')/'console'/'index.html'; print('yes' if p.is_file() else 'no')" 2>/dev/null || echo 'no')"
...
cat > "$TQCLAW_BIN/tqclaw" << 'WRAPPER'
#!/usr/bin/env bash
# TQ-Claw CLI wrapper — delegates to the uv-managed environment.
set -euo pipefail

TQCLAW_HOME="${TQCLAW_HOME:-$HOME/.tqclaw}"
REAL_BIN="$TQCLAW_HOME/venv/bin/tqclaw"
...
chmod +x "$TQCLAW_BIN/tqclaw"
info "Wrapper created at $TQCLAW_BIN/tqclaw"

# ── Step 5: Update PATH in shell profile ─────────────────────────────────────
PATH_ENTRY="export PATH=\"\$HOME/.tqclaw/bin:\$PATH\""

add_to_profile() {
    local profile="$1"
    if [ -f "$profile" ] && grep -qF '.tqclaw/bin' "$profile"; then
...
# ── Done ──────────────────────────────────────────────────────────────────────
echo ""
printf "${GREEN}${BOLD}TQ-Claw installed successfully!${RESET}\n"
echo ""

# Install summary
printf "  Install location:  ${BOLD}%s${RESET}\n" "$TQCLAW_HOME"
printf "  Python:            ${BOLD}%s${RESET}\n" "$("$TQCLAW_VENV/bin/python" --version 2>&1)"
...
echo "Then run:"
echo ""
printf "  ${BOLD}tqclaw init${RESET}       # first-time setup\n"
printf "  ${BOLD}tqclaw app${RESET}        # start TQ-Claw\n"
echo ""
printf "To upgrade later, re-run this installer.\n"
printf "To uninstall, run: ${BOLD}tqclaw uninstall${RESET}\n"

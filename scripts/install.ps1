# TQ-Claw Installer for Windows (self-contained: includes uv download via GitHub)
# Usage: irm <url>/install.ps1 | iex
#    or: .\install.ps1 [-Version X.Y.Z] [-FromSource] [-SourceDir DIR]
#                            [-Extras "llamacpp,mlx"] [-UvPath PATH]
#
# Installs TQ-Claw into ~/.tqclaw with a uv-managed Python environment.
...
# ── Defaults ──────────────────────────────────────────────────────────────────
$TqclawHome     = if ($env:TQCLAW_HOME) { $env:TQCLAW_HOME } else { Join-Path $HOME ".tqclaw" }
$TqclawVenv     = Join-Path $TqclawHome "venv"
$TqclawBin      = Join-Path $TqclawHome "bin"
$PythonVersion = "3.12"
$TqclawRepo     = "https://github.com/agentscope-ai/TQ-Claw.git"

# ── Colors ────────────────────────────────────────────────────────────────────
function Write-Info { param([string]$Message) Write-Host "[tqclaw] " -ForegroundColor Green  -NoNewline; Write-Host $Message }
function Write-Warn { param([string]$Message) Write-Host "[tqclaw] " -ForegroundColor Yellow -NoNewline; Write-Host $Message }
function Write-Err  { param([string]$Message) Write-Host "[tqclaw] " -ForegroundColor Red    -NoNewline; Write-Host $Message }
function Stop-WithError { param([string]$Message) Write-Err $Message; exit 1 }

# ── Help ──────────────────────────────────────────────────────────────────────
if ($Help) {
    @"
TQ-Claw Installer for Windows
...
Environment:
  TQCLAW_HOME            Installation directory (default: ~/.tqclaw)
"@
    exit 0
}

Write-Host "[tqclaw] " -ForegroundColor Green -NoNewline
Write-Host "Installing TQ-Claw into " -NoNewline
Write-Host "$TqclawHome" -ForegroundColor White
...
function Prepare-Console {
    ...
    $consoleDest = Join-Path $RepoDir "src\tqclaw\console"
...
$VenvTqclaw = Join-Path $TqclawVenv "Scripts\tqclaw.exe"

if ($FromSource) {
    if ($SourceDir) {
        $SourceDir = (Resolve-Path $SourceDir).Path
        Write-Info "Installing TQ-Claw from local source: $SourceDir"
        ...
        if (-not (Get-Command git -ErrorAction SilentlyContinue)) {
            Stop-WithError "git is required for -FromSource without a local directory. Please install Git from https://git-scm.com/ or pass a local path: .\install.ps1 -FromSource -SourceDir C:\path\to\TQ-Claw"
        }
        Write-Info "Installing TQ-Claw from source (GitHub)..."
        $cloneDir = Join-Path $env:TEMP "tqclaw-install-$(Get-Random)"
        try {
            git clone --depth 1 $TqclawRepo $cloneDir
    ...
    $package = "tqclaw"
    if ($Version) { $package = "tqclaw==$Version" }

    Write-Info "Installing ${package}${ExtrasSuffix} from PyPI..."
    uv pip install "${package}${ExtrasSuffix}" --python $VenvPython --prerelease=allow --quiet --refresh-package tqclaw
    if ($LASTEXITCODE -ne 0) { Stop-WithError "Installation failed" }
}

# Verify the CLI entry point exists
if (-not (Test-Path $VenvTqclaw)) { Stop-WithError "Installation failed: tqclaw CLI not found in venv" }

Write-Info "TQ-Claw installed successfully"

# Check console availability (for PyPI installs, check the installed package)
if (-not $script:ConsoleAvailable) {
    $consoleCheck = & $VenvPython -c "import importlib.resources, tqclaw; p=importlib.resources.files('tqclaw')/'console'/'index.html'; print('yes' if p.is_file() else 'no')" 2>&1
    if ($consoleCheck -eq "yes") { $script:ConsoleAvailable = $true }
}

# ── Step 4: Create wrapper scripts ───────────────────────────────────────────
New-Item -ItemType Directory -Path $TqclawBin -Force | Out-Null

$wrapperPath = Join-Path $TqclawBin "tqclaw.ps1"
$wrapperContent = @'
# TQ-Claw CLI wrapper — delegates to the uv-managed environment.
$ErrorActionPreference = "Stop"

$TqclawHome = if ($env:TQCLAW_HOME) { $env:TQCLAW_HOME } else { Join-Path $HOME ".tqclaw" }
$RealBin   = Join-Path $TqclawHome "venv\Scripts\tqclaw.exe"

if (-not (Test-Path $RealBin)) {
    Write-Error "TQ-Claw environment not found at $TqclawHome\venv"
...
$cmdWrapperPath = Join-Path $TqclawBin "tqclaw.cmd"
$cmdWrapperContent = @"
@echo off
REM TQ-Claw CLI wrapper — delegates to the uv-managed environment.
set "TQCLAW_HOME=%TQCLAW_HOME%"
if "%TQCLAW_HOME%"=="" set "TQCLAW_HOME=%USERPROFILE%\.tqclaw"
set "REAL_BIN=%TQCLAW_HOME%\venv\Scripts\tqclaw.exe"
if not exist "%REAL_BIN%" (
    echo Error: TQ-Claw environment not found at %TQCLAW_HOME%\venv >&2
...
if (-not $isAlreadyAdded) {
    ...
    try {
        ...
        Write-Host ""
        Write-Host "ACTION REQUIRED: You must manually add the path to use TQ-Claw."
...
# ── Done ──────────────────────────────────────────────────────────────────────
Write-Host ""
Write-Host "TQ-Claw installed successfully!" -ForegroundColor Green
Write-Host ""

Write-Host "  Install location:  " -NoNewline; Write-Host "$TqclawHome" -ForegroundColor White
...
Write-Host ""
Write-Host "  tqclaw init" -ForegroundColor White -NoNewline; Write-Host "       # first-time setup"
Write-Host "  tqclaw app"  -ForegroundColor White -NoNewline; Write-Host "        # start TQ-Claw"
Write-Host ""
Write-Host "To upgrade later, re-run this installer."
Write-Host "To uninstall, run: " -NoNewline
Write-Host "tqclaw uninstall" -ForegroundColor White

...
- Builds the console frontend (`console/`), copies `console/dist` to `src/tqclaw/console/dist`, then builds the wheel. Output: `dist/*.whl`.
...
- Default tag: `tqclaw:latest`. Uses `deploy/Dockerfile` (multi-stage: builds console then Python app).
- Example: `bash scripts/docker_build.sh myreg/tqclaw:v1 --no-cache`.
